"""
MHA Inference Example with KV Splitting

Demonstrates Multi-Head Attention inference pattern with KV sequence splitting
for improved parallelism on Ascend NPU. Uses TileLang to optimize:
- Split KV processing across multiple kernels
...
def flashattn_mha_inference_split_kv(batch, heads, seqlen_q, seqlen_kv, dim, num_split=DEFAULT_NUM_SPLIT, block_M=128, block_N=128):
    kv_per_split = seqlen_kv // num_split

    kernels = []
    workspaces = []

    for _sid in range(num_split):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_split", type=int, default=DEFAULT_NUM_SPLIT)
