...
def cpu_lightning_indexer(
    ...
    *,
    ...
    # Full score matrix [B, N2, S1, max(ask)] for AscendC-style 3-step verification.
    # float32 matches kernel calc_dtype; masked positions stay -inf.
...
def make_lightning_indexer_kernel(
    ...
    if G <= 32 and G % 8 == 0:
        VECTOR_BASEG = G  # full fuse: num_g_groups = 1 (e.g. G=24 -> VG=24)
    elif G % 32 == 0:
        VECTOR_BASEG = 32
    elif G % 16 == 0:
        VECTOR_BASEG = 16
    ...
    def kernel_func():
        ...
        def main(
            ...
            with T.Kernel(core_num, is_npu=True) as (cid, vid):
                ...
                # Scalar clamp buffer: stores QOffset+s1_idx (which involves long-typed
                # cid/vid) as int32, so T.min() doesn't hit CCE min(long,int) ambiguity.
                row_clamp_ub = T.alloc_ub(1, "int32")
                            ...
                            for s1_local in range(S1_BLOCK):
                                ...
                                for g_sub in range(M_SUB_TILES):
                                    ...
                                    if g_end > G:
                                        g_end = G
                                    if is_tnd:
                                        # Clamp: if last batch has _q_len==0, s1_idx=0
                                        # and QOffset[b_idx]=q_tot → OOB on Query.
                                        # T.if_then_else used instead of T.min because
                                        # s1_idx is a T.if_then_else result (condval),
                                        # which breaks UB SetValue codegen. Functional
                                        # equivalent of T.min(expr, _q_tot-1).
                                        _q_row = T.if_then_else(QOffset[b_idx] + s1_idx < _q_tot, QOffset[b_idx] + s1_idx, _q_tot - 1)
                                        T.copy(
                                            Query[_q_row, n2_idx * G + g_start : n2_idx * G + g_end, 0:D],
                ...
                with T.Scope("V"):
                    ...
                    for bsn_off in range(bsn_groups):
                        ...
                        if prev_bsn_ub[0] >= 0 and blk_bsn != prev_bsn_ub[0]:
                            ...
                            for _si in range(VID_S1):
                                # Guard: skip phantom rows when S1_BLOCK is odd
                                if vid * VID_S1 + _si < S1_BLOCK:
                                    T.copy(
                                        topk_a_ub[_si, :],
                                        TopK_Workspace[
                        ...
                        for s2_local in T.serial(s2_per_group):
                            ...
                            # Ensure cross-flag is fully consumed before any MTE2
                            # QK read — prevents timing race where MTE2 reads GM
                            # before the FIX pipe's L0C→GM write is visible.
                            T.pipe_barrier("V")
                            ...
                            if group_task_id + s2_local < block_num:
                                ...
                                for s1_local in range(VID_S1):
                                    ...
                                    # Guard: when S1_BLOCK is odd, VID_S1 = (S1_BLOCK+1)//2
                                    # exceeds S1_BLOCK//2, so vid=1 may produce
                                    # actual_s1 >= S1_BLOCK → OOB on QK_Workspace
                                    # (dim S1_BLOCK*G_padded). Skip phantom rows.
                                    if actual_s1 < S1_BLOCK and s1_start + actual_s1 < _q_len_b:
                                        ...
                                        if s2_start < s2_valid:
                                            ...
                                            if num_g_groups == 1:
                                                ...
                                                if is_tnd:
                                                    row_clamp_ub[0] = QOffset[b_idx] + s1_idx
                                                    _w_row = T.min(row_clamp_ub[0], _q_tot - 1)
                                            ...
                                            if _sc == 2:
                                                ...
                                                # Clear slots after 4-way merge so stale data
                                                # doesn't leak into subsequent tail merges.
                                            ...
                                            # Tail merge at last s2_local: end-of-loop OR BSN boundary.
                                            # BSN-local _sc fixes slot overwrite within a BSN.
                                            # Cross-BSN tail merge: at last s2_local, check if next bsn_off
                                            # is a different BSN. If so, merge remaining slots here (inside
                                            # s2_local loop where merged_ub is already live — no UB overflow).
                                            if s2_local == s2_per_group - 1 and (
                                                (bsn_off == bsn_groups - 1)
                                                | ((cid * tasks_per_core + (bsn_off + 1) * s2_per_group) // num_s2_blocks != blk_bsn)
                                            ):
                                                if _sc == 1:
                                                    T.pipe_barrier("V")
                                            ...
                                            if _sc == 2:
                                                ...
                                                T.tile.fill(cache_slot1_ub[s1_local, :], -T.infinity(calc_dtype))
                                                T.tile.fill(cache_slot2_ub[s1_local, :], -T.infinity(calc_dtype))
                                            # Tail merge for skip blocks: end-of-loop OR BSN boundary
                                            if s2_local == s2_per_group - 1 and (
                                                (bsn_off == bsn_groups - 1)
                                                | ((cid * tasks_per_core + (bsn_off + 1) * s2_per_group) // num_s2_blocks != blk_bsn)
                                            ):
                    ...
                    if prev_bsn_ub[0] >= 0:
                        ...
                        for s1_local in range(VID_S1):
                            # Guard: skip phantom rows when S1_BLOCK is odd
                            if vid * VID_S1 + s1_local < S1_BLOCK:
                                T.copy(
                                    topk_a_ub[s1_local, :],
                                    TopK_Workspace[
                    ...
                    # CRITICAL: Must wait for MTE3 (UB→GM write) to complete before
                    # SyncAll, otherwise Phase 2 reads stale TopK_Workspace data.
                    # pipe_barrier("V") only syncs the V pipe — MTE3 writes from the
                    # final flush above may still be in the DMA buffer. barrier_all
                    # (PipeBarrier<PIPE_ALL>) ensures ALL pipes including MTE3 finish.
                    T.barrier_all()
                    T.sync_all()
                ...
                for row_off in T.serial(rows_per_core_p2):
                    ...
                    if out_row < num_output_rows:
                        ...
                        if s1_idx_p2 < _q_len_p2 and out_s1_local < S1:
                            ...
                            T.pipe_barrier("V")
                            # score_topk_ub now holds the values extracted from p2_acc_ub.
                            # After compare, score_topk_ub is no longer needed for the index
                            # path — reuse it for value output to avoid re-reading p2_acc_ub
                            # (which may be corrupted by select writing -1.0 to topk_index_ub
                            # if the memory planner assigned them overlapping UB regions).
                            ...
                            if return_value:
                                ...
                                # Cast values from score_topk_ub (saved before select)
                                # instead of re-gathering from p2_acc_ub.
                            ...
                            # Ensure MTE3 output write completes before next row_off
                            # iteration starts. Without this, MTE3 from iter N may
                            # still be in DMA buffer when iter N+1 issues MTE2 read,
                            # causing non-deterministic output corruption.
                            T.barrier_all()
                        else:
                            # s1 >= act_q or out_s1_local >= S1 — fill -1 for BSND
                            # For TND, skip writing to avoid overwriting next batch's data
                            # in the packed flat tensor (Out is already -1 initialized by host).
                            if not is_tnd:
