...
def sparse_attention_fwd(
    ...
    def main(
        ...
        T.func_attr({"kernel_C_V_1_1": True})
        # VEC_NUM = 1
        with T.Kernel(block_num, is_npu=True) as (cid, vid):
            ...
            kv_ub = T.alloc_ub([BI, D], dtype) 
            kv_tail_ub = T.alloc_ub([BI, D_tail], dtype) 
            ...
            for i_i in range(NI):
                ...
                for bi_i in range(BI // VEC_NUM):
                    T.copy(KV[b_i, indices_ub_[bi_i + vid * BI // VEC_NUM], g_i, :D], kv_ub[bi_i + vid * BI // VEC_NUM, :])
                    T.copy(KV[b_i, indices_ub_[bi_i + vid * BI // VEC_NUM], g_i, D:], kv_tail_ub[bi_i + vid * BI // VEC_NUM, :])
                T.copy(kv_ub, kv_l1)            
                T.copy(kv_tail_ub, kv_tail_l1)  
...
def ref_sparse_attention_fwd_interface(q,
    ...
    dim = q.shape[-1] - kv.shape[-1] + q.shape[-1] if kv.shape[-1] != q.shape[-1] else 128
    k = kv
    v = kv[..., :dim]
...
B, S, SKV, H, HKV, DQK, DV, topk = 1, 32, 2048, 32, 1, 144, 128, 512
dtype = torch.float16

KV_stride = 1
q_start_s_index = 512 * 3  
...
for b in range(B):
    for t in range(S):
        for h in range(HKV):
            max_idx = max(1, ((t + q_start_s_index) // KV_stride))
            i_i = torch.randperm(max_idx)[:min(topk, max_idx)]
