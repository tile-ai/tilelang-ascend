...
def sparse_attention_fwd(
    ...
    def main_broadcast(
        ...
        with T.Kernel(core_num, is_npu=True) as (cid, vid):
            ...
            # indices_ub_ = T.alloc_ub([1, BI], indices_dtype)
            indices_ub_float = T.alloc_ub([BI], "float")
            # indices_ub_float = T.alloc_ub([1, BI], "float")
            ...
            # mask_ub = T.alloc_ub([1, BI // 8], "uint8")
            # mask_ub_2d = T.alloc_ub([v_block, BI // 8], "uint8")
            mask_ub_2 = T.alloc_ub([BI // 8], "uint8")
            # mask_ub_2 = T.alloc_ub([1, BI // 8], "uint8")
                    ...
                    if s_i < act_q_len:
                        ...
                        # for i_i in T.Pipelined(NI, num_stages=2):
                            ...
                            for bi_i in range(BI // 2):
                                ...
                                # index_i = indices_ub_[0, bi_i + vid * BI // 2]
                            ...
                            # T.tile.broadcast(mask_ub_2d, mask_ub, tmp_ub)
                            # T.tile.select(acc_s_ub, mask_ub_2d, acc_s_ub_, -T.infinity(accum_dtype), "VSEL_TENSOR_SCALAR_MODE")
                            ...
                            # replace
                            # m_i is [v_block, 1], broadcast to [v_block, BI] (axis=1, broadcast along columns)
                            ...
                            # replace
                            # m_i_prev is [v_block, 1], broadcast to [v_block, D] (axis=1)
                        ...
                        # replace
                        # sumexp is [v_block, 1], broadcast to [v_block, D] (axis=1)
