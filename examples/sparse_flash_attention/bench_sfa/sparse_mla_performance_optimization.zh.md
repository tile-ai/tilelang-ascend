...
- **计算任务固定映射到物理核心**：由于 A2、A3 机器在物理上有 24 个 AI Core，不论输入产生多少个逻辑计算块，都通过静态负载均衡的方式将其分配给固定的 24 个物理核执行，避免计算任务规模无限制展开。
...
- **seq_len_kv核内切分**：提升 `seq_len_kv` 维度分块大小，增加每轮 Gather kv 和 Copy out 的数据规模，有利于提升访存吞吐。
