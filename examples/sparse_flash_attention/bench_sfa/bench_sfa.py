...
def test_op(T, B, KV_S, Q_N, KV_N, D, D_rope,
            ...
            sparse_block_size, sparse_mode, block_size, act_kv_s):
    ...
    tilelang_out = sparse_attn_tilelang(
        query = query,
        key = key,
        value = value,
        sparse_indices = sparse_indices,
    ...
    tilelang_out = tilelang_out.to(npu_out.dtype)
    torch.testing.assert_close(tilelang_out, npu_out, rtol=1e-2, atol=1e-2, equal_nan=True)
    print("[PASSED]")
