...
def sparse_attention_fwd(
    ...
    def main(
        ...
        with T.Kernel(core_num, is_npu=True) as (cid, vid):
            ...
            kv_ub = T.alloc_ub([v_block, D], dtype)
            kv_tail_ub = T.alloc_ub([v_block, D_tail], dtype)
                    ...
                    with T.Scope("V"):
                        ...
                        for i_i in range(NI):
                            ...
                            for bi_i in range(BI // 2):
                                T.copy(KV[b_i, indices_ub_[bi_i + vid * BI // 2], g_i, :D], kv_ub[bi_i: bi_i+1, :])
                                T.copy(KV[b_i, indices_ub_[bi_i + vid * BI // 2], g_i, D:], kv_tail_ub[bi_i: bi_i+1, :])
                            T.barrier_all()
                            T.copy(kv_ub, workspace_1[cid, vid * BI // 2 : (vid + 1) * BI // 2, :])
                            T.copy(kv_tail_ub, workspace_2[cid, vid * BI // 2 : (vid + 1) * BI // 2, :])
                            T.barrier_all()
