    ...
    def normalize_weight_kernel(
        ...
        with T.Kernel(num_cores, is_npu=True) as (cid, vid), T.Scope("V"):
            ...
            if cid < num_token_blocks:
                ...
                T.copy(topk_weights[token_start : token_start + rows_per_vec, 0:num_topk], weights_ub[0, :, :], pad_value=0.0)
                T.set_flag("mte2", "v", 0)
            ...
            for i in T.serial(num_iters):
                ...
                if block_id < num_token_blocks:
                    ...
                    T.copy(normalized_ub[:, :num_topk], normalized_weights[token_start : token_start + rows_per_vec, :])
...
def main():
    ...
    for num_tokens, num_topk in get_test_configs():
        ...
        case = f"normalize_weight T={num_tokens} K={num_topk}"
        print(f"pass {case}")
