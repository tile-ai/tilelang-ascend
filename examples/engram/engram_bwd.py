    ...
    def _func1(threads):
        ...
        def _bwd_pass1(
            ...
            with T.Kernel(seq_len * M, is_npu=True, threads=[threads]) as (cid, _):
                ...
                dy_local_cast = T.alloc_shared((1, d_padded), dtype)
                dy_local = T.alloc_shared((1, d_padded), accum_dtype)
                T.copy(dY[bid, tid, :], dy_local_cast)
                T.vcast(dy_local_cast, dy_local)
                conv_out = T.alloc_fragment((1, d_padded), accum_dtype)
                T.clear(conv_out)
                ...
                d_silu = T.alloc_shared((1, d_padded), accum_dtype)
    ...
    def _func2(threads):
        ...
        def _bwd_pass2(
            ...
            with T.Kernel(seq_len * M, is_npu=True, threads=[threads]) as (cid, _):
                bid = cid % M
                tid = cid // M
                d_vnorm = T.alloc_shared((1, d_padded), accum_dtype)
                T.clear(d_vnorm)
                for p in T.serial(KS):
                    dst_t = tid + (KS - 1) - p
                    d_si = T.alloc_shared((1, d_padded), accum_dtype)
                    T.clear(d_si)
                    if (dst_t >= 0) * (dst_t < seq_len):
                        T.copy(dvhat_buf[bid, dst_t : dst_t + 1, :], d_si)
                    conv_w_cast = T.alloc_shared((1, d_padded), dtype)
                    conv_w_shared = T.alloc_shared((1, d_padded), accum_dtype)
                    T.copy(conv_w[p : p + 1, 0:d_padded], conv_w_cast)
                    T.vcast(conv_w_cast, conv_w_shared)
                    for j in T.Parallel(d_padded):
                        d_vnorm[0, j] += conv_w_shared[0, j] * d_si[0, j]

                rrms_v_val = rrms_v[bid, tid]
                vhat_local_cast = T.alloc_shared((1, d_padded), dtype)
                vhat_local = T.alloc_shared((1, d_padded), accum_dtype)
                T.copy(vhat[bid, tid : tid + 1, :], vhat_local_cast)
                T.vcast(vhat_local_cast, vhat_local)
                vhat_2d = T.alloc_shared((1, d_padded), accum_dtype)
                T.copy(vhat_local, vhat_2d)

                add_val = T.alloc_shared((1, d_padded), accum_dtype)
                ...
                rms_w_v_cast = T.alloc_shared((d_padded,), dtype)
                rms_w_v_shared = T.alloc_shared((d_padded,), accum_dtype)
                rms_w_v_reshape = T.alloc_shared((1, d_padded), accum_dtype)
                T.copy(rms_w_v, rms_w_v_cast)
                T.vcast(rms_w_v_cast, rms_w_v_shared)
                T.reshape(rms_w_v_shared, rms_w_v_reshape)
                for j in T.Parallel(d_padded):
                    dot_2d[0, j] = (
                        vhat_local[0, j] * rms_w_v_reshape[0, j] * d_vnorm[0, j]
                    )
                ...
                for j in T.serial(d_padded):
                    dvhat_local[0, j] = (
                        rrms_v_val * rms_w_v_reshape[0, j] * d_vnorm[0, j]
                        - rrms_v_val
                        * rrms_v_val
                        * rrms_v_val
                        * vhat_local[0, j]
    ...
    def _func3(threads):
        ...
        def _bwd_pass3(
            ...
            with T.Kernel(seq_len * M, is_npu=True, threads=[threads]) as (cid, _):
                ...
                v_local_cast = T.alloc_shared((1, d_padded), dtype)
                v_local = T.alloc_shared((1, d_padded), accum_dtype)
                T.copy(v[bid, tid, :], v_local_cast)
                T.vcast(v_local_cast, v_local)
                h_local_cast = T.alloc_shared((1, d_padded), dtype)
                h_local = T.alloc_shared((1, d_padded), accum_dtype)
                T.copy(H[bid, tid, :], h_local_cast)
                T.vcast(h_local_cast, h_local)
                k_local_cast = T.alloc_shared((1, d_padded), dtype)
                k_local = T.alloc_shared((1, d_padded), accum_dtype)
                ...
                for j in T.Parallel(d_padded):
                    h_norm_local[j] = h_local[0, j] * rrms_h_val * rms_w_h_local[j]
                    k_norm_local[j] = k_local[0, j] * rrms_k_val * rms_w_h_local[j]
                dot_h_2d = T.alloc_shared((1, d_padded), accum_dtype)
                for j in T.serial(d_padded):
                    dot_h_2d[0, j] = (
                        h_local[0, j] * rms_w_h_local[j] * ddot_val * k_norm_local[j]
                    )
                ...
                for j in T.serial(d_padded):
                    add_val[j] = ddot_val * k_norm_local[j] * h_local[0, j] * rrms_h_val
                T.atomic_add(drms_w_h, add_val)

                dot_k_2d = T.alloc_shared((1, d_padded), accum_dtype)
                for j in T.serial(d_padded):
                    dot_k_2d[0, j] = (
                        k_local[0, j] * rms_w_h_local[j] * ddot_val * h_norm_local[j]
                    )
                ...
                for j in T.serial(d_padded):
                    add_val_2[j] = (
                        ddot_val * h_norm_local[j] * k_local[0, j] * rrms_k_val
                    )
                T.atomic_add(drms_w_h, add_val_2)

                dh_val = T.alloc_shared((d_padded,), dtype)
                T.vcast(dh_local, dh_val)
                dk_val = T.alloc_shared((d_padded,), dtype)
                T.vcast(dk_local, dk_val)
                dv_val = T.alloc_shared((d_padded,), dtype)
