            ...
            if cid < SHARED_CORES:
                ...
                with T.Scope("C"):
                    ...
                    for i in T.serial(PRE_LAUNCH):
                        ...
                        T.copy(
                            K_unshared[
                                kv_row_base : kv_row_base + KV_BATCH,
                                :D,
                            ],
                    ...
                    for i in T.serial(PRE_LAUNCH, TASKS_PER_CORE + PRE_LAUNCH):
                        side = i % TASKQUE_SLOTS
                        ...
                        if i >= PRE_LAUNCH:
                            ...
                            T.copy(
                                V_unshared[
                                    prev_kv_row_base : prev_kv_row_base + KV_BATCH,
                                    :D,
                                ],
                                v_l1_u[0:KV_BATCH, :D],
                            )
                        ...
                        if i >= PRE_LAUNCH:
                            ...
                            if prev_task_idx < TOTAL_UNSHARED_TASKS:
                                ...
                                T.copy(
                                    mma_l0c[pv_l0c_slot, :, :],
                                    unshared_O_flat[
                                        prev_o_row_base : prev_o_row_base + Q_BATCH,
                                        :D,
                                    ],
                                )
                ...
                with T.Scope("V"):
                    for i in T.serial(TASKS_PER_CORE):
                        ...
                        for unshared_sub_chunk in T.serial(
                            ...
                            unshared_col = (
                                unshared_sub_chunk * UNSHARED_VECTOR_ELEMS
                            )
                            T.tile.sub_mask(
                                unshared_s[
                                    :,
                                    unshared_col : unshared_col
                                    + UNSHARED_VECTOR_ELEMS,
                                ],
                                unshared_s[
                                    :,
                                    unshared_col : unshared_col
            ...
            with T.Scope("V"):
                ...
                for merge_slot in T.serial(MERGE_TILES_PER_WORKER):
                    if merge_slot < merge_worker_tile_count:
                        ...
                        T.pipe_barrier("v")
                        T.tile.mul_mask(
                            merge_s_O[merge_buf, :, 0:MERGE_FLOAT_VECTOR_ELEMS],
                            merge_s_O[merge_buf, :, 0:MERGE_FLOAT_VECTOR_ELEMS],
                            merge_s_gm_lane,
                            [MERGE_FLOAT_MASK],
                            MERGE_ROW_TILE,
                        ...
                        T.tile.mul_mask(
                            merge_s_O[merge_buf, :, MERGE_FLOAT_VECTOR_ELEMS:D],
                            merge_s_O[merge_buf, :, MERGE_FLOAT_VECTOR_ELEMS:D],
                            merge_s_gm_lane,
                            [MERGE_FLOAT_MASK],
                            MERGE_ROW_TILE,
                        ...
                        T.pipe_barrier("v")
                        T.tile.mul_mask(
                            merge_u_O[merge_buf, :, 0:MERGE_FLOAT_VECTOR_ELEMS],
                            merge_u_O[merge_buf, :, 0:MERGE_FLOAT_VECTOR_ELEMS],
                            merge_s_gl_lane,
                            [MERGE_FLOAT_MASK],
                            MERGE_ROW_TILE,
                        ...
                        T.tile.mul_mask(
                            merge_u_O[merge_buf, :, MERGE_FLOAT_VECTOR_ELEMS:D],
                            merge_u_O[merge_buf, :, MERGE_FLOAT_VECTOR_ELEMS:D],
                            merge_s_gl_lane,
                            [MERGE_FLOAT_MASK],
                            MERGE_ROW_TILE,
...
if __name__ == "__main__":
    ...
    print(f"Testing x_attention decode: beam={BEAM_SIZE}, kv_len={KV_LEN}")
    print(f"  BUILD_TAG={BUILD_TAG}")
    print(
        "  light S/P/O GM slots: "
        f"S={SHARED_S_SLOTS}, P={SHARED_P_SLOTS}, O={SHARED_O_SLOTS}, "
        f"taskque={TASKQUE_SLOTS}"
    )
    print(
        "  unshared tasks: "
        f"total={TOTAL_UNSHARED_TASKS}, tasks/core={TASKS_PER_CORE}, "
        f"q_batch={Q_BATCH}, kv_batch={KV_BATCH}, decode_step={DECODE_STEP}"
    )
    print(
        "  shared tasks: "
        f"pairs={SHARED_TOTAL_PAIRS}, kv_tiles={SHARED_KV_TILES}, "
        f"tasks/core={SHARED_TASKS_PER_CORE}, pipe_steps={SHARED_PIPE_STEPS}"
    )
    if _is_simulator():
        Q = torch.full(
    ...
    kernel_dir = os.path.join(os.getcwd(), "xakernel")
    os.makedirs(kernel_dir, exist_ok=True)
    kernel_name = (
        f"xa_v4_direct_unshared_o_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.cpp"
    )
    kernel_path = os.path.join(kernel_dir, kernel_name)
    latest_kernel_path = os.path.join(kernel_dir, "latest.cpp")
    with open(kernel_path, "w", encoding="utf-8") as f:
        f.write(src)
    with open(latest_kernel_path, "w", encoding="utf-8") as f:
        f.write(src)
