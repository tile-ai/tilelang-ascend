...
def fp16_gemm(
    ...
    Accumulation: FP32 (must use FP32 accumulation)
    ...
    dtype = "float16"
    accum_dtype = "float"  # Must use FP32 accumulation!
    ...
    def main(
        ...
        with T.Kernel(m_num * n_num, is_npu=True) as (cid, _):
            ...
            C_L0 = T.alloc_fragment((block_M, block_N), accum_dtype)  # FP32 accumulation!
...
def dequant_gemm_wrapper(
    ...
    """
    Dequantize GEMM operator wrapper
    ...
    Implementation: Python-side FP4->FP16 + BF16->FP16 + NPU-side FP16xFP16 GEMM + FP32 computation precision

    Args:
        M, N, K: matrix dimensions
        block_M, block_N, block_K: block sizes
        input_dtype: input data type (bfloat16/float16)
    ...
    def wrapper(A: torch.Tensor, B_packed: torch.Tensor):
        ...
        B_fp16 = torch_unpack_fp4_to_fp16(B_packed)  # (N, K)

        # 2. Convert input A to FP16
        if input_dtype == "bfloat16":
            A_fp16 = A.to(torch.float16)  # (M, K)
        else:
            A_fp16 = A  # Already float16
...
def ref_program(A: torch.Tensor, B_packed: torch.Tensor, output_dtype: str):
    ...
    Args:
        A: torch.Tensor, shape (M, K), dtype bfloat16/float16
...
def check_case(
    ...
    # Execute kernel
    wrapper = dequant_gemm_wrapper(M, N, K, block_M, block_N, block_K, input_dtype, output_dtype)
...
def main(custom_args=None):
    ...
    """
    parser = argparse.ArgumentParser(description="Dequantize GEMM Example (FP4 -> FP16)")
    ...
    print("=" * 60)
    print("Dequantize GEMM Tests (FP4 -> FP16)")
    print("=" * 60)

    # Level 0: small-scale test
    check_case(256, 256, 256, block_M=128, block_N=256, block_K=64, input_dtype=args.input_dtype, output_dtype="bfloat16")

    # Level 1: typical-scale test
    check_case(512, 512, 512, block_M=128, block_N=256, block_K=64, input_dtype=args.input_dtype, output_dtype="bfloat16")

    # Level 1: large-scale test
    check_case(M, N, K, block_M=128, block_N=256, block_K=64, input_dtype=args.input_dtype, output_dtype="bfloat16")
