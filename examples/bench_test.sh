...
# 1. 收集脚本逻辑 (保持原样)
python_files=$(find . -maxdepth 2 -name "*.py" -not -path "./gemm_aot/*" -not -path "./dispatch_combine/*" -not -path "./shmem/*" -not -path "./torch_tl_ascend/*" -not -name "sfa_golden.py" -not -name "__init__.py" -not -path "./lightning_indexer/*" -not -path "./topk_selector/*" | sort)
