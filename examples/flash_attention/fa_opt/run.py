...
def run_and_analyze(cmd, output_path, kernel_name, desc, timeout=None):
    ...
    """
    print(f"  > 正在执行 {desc} (Timeout={timeout})..." if timeout else f"  > 正在执行 {desc}...")

    # 使用 start_new_session=True 创建新的进程组，以便能够发送信号给整个组
    ...
    try:
        ...
        print(f"    [!] {desc} 超时 (>{timeout}s)，发送 SIGINT，等待数据生成...")
        # 发送 SIGINT 给进程组
        os.killpg(os.getpgid(process.pid), signal.SIGINT)
        
        # 等待进程响应信号并退出，不再强制 Kill，因为生成性能数据可能很慢
    ...
    # 只有当返回码非0 (且非超时) 或者 发现了明确的错误关键字，才认为是 Fail
    # 超时情况下，MSPFOF 可能会生成有效数据，但也可能返回非0码
    if (returncode != 0 and status != "Timeout") or has_error_keyword:
        ...
        # 如果是 Timeout 但有错误信息，追加显示
        if status == "Timeout":
             error_info = f"Timeout & {error_info}"

        print(f"    [X] {desc} 执行异常! 状态: {status}, 错误: {error_info}")

        print("-" * 30 + " 完整报错日志 " + "-" * 30)
        print(full_output)
        print("-" * 74)
    elif status == "Timeout":
        print(f"    [!] {desc} 执行超时，已尝试捕获性能数据。")
...
def run_performance_case(pack):
    """
    Performance 模式下，顺序执行 TileLang 和 AscendC，并直接返回 CSV 行
    """
...
def run_sim_task(pack):
    """
    Simulation 模式下，执行单个任务 (TileLang 或 AscendC)
    ...
    if task_type == 'TL':
        ...
        exec_time, status, error_info = run_and_analyze(cmd_ac, path_ac, args.kernel_ascendc, f"AscendC [{combo_name}]", timeout=args.timeout)
...
def run_benchmark():
    ...
    parser.add_argument("--timeout", type=float, default=None, help="Timeout in seconds for each msprof execution (sends SIGINT on timeout)")
    ...
    if args.sim:
        print(f">>> 启用多进程 Simulation 模式 (Workers: {args.workers})")
        # 1. 扁平化任务列表
        sim_tasks = []
        ...
        # 提前准备 rows cache，以便最后按顺序写入
        # 但为了实时打印，我们需要知道何时一个 case 的 TL 和 AC 都完成了
