...
extern "C" __global__ __aicore__ void main_kernel( GM_ADDR A_handle,  GM_ADDR B_handle,  GM_ADDR C_handle, uint64_t fftsAddr) {
  ...
  if ASCEND_IS_AIC {
    for (int32_t k = 0; k < 128; ++k) {
      tl::ascend::copy_gm_to_l1<half, 128, 64>(A_L1[0], A[(((cid / 4) * 1048576) + (k * 64))], 8192, 128, 64);
      tl::ascend::copy_gm_to_l1<half, 64, 256>(B_L1[0], B[((k * 65536) + ((cid % 4) * 256))], 1024, 64, 256);
      AscendC::PipeBarrier<PIPE_ALL>();
      tl::ascend::gemm_v0<half, float, 128, 256, 64, false, false>(A_L1[0], B_L1[0], C_L0[0], ascend_l0a, ascend_l0b, (k == 0));
      AscendC::PipeBarrier<PIPE_ALL>();
    }
    tl::ascend::copy_l0c_to_gm<float, half, layout::RowMajor, 128, 256, 0>(C[(((cid / 4) * 131072) + ((cid % 4) * 256))], C_L0[0], 1024, 128, 256);
  }
}
