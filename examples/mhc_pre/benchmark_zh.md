...
| 测试移入 pytest | 批量 shape 测试移至 test_example_mhc_pre.py，注册进 operator_test_manifest；example 只留单 shape 用例 | 纳入 CI；17 个 shape + distinct-eps（共 18 个测试），low_priority 用例仅在 full/scheduled 任务跑 |
| CI 用例预算 | 默认（PR）集按 kernel 编译键压缩到 5 个 case（每次 PR 的 kernel 编译从 42 次降至 15 次），其余 13 个 shape 标 low_priority | 修复 Benchmark Tests 60 分钟超时：--forked 使每个 case 冷编译 3 个 kernel，且 manifest 变更触发全量 CI；参照 moe/quant 先例（#1711） |
