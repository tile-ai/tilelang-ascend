...
@tilelang.jit()
def shmem_get(M, N, nelems, newPe, dtype="float16"):
    @T.prim_func
    def main(
...
def worker(rank, barrier, rank_table_path):
    ...
    if result == 0:
        ...
        tensor = aclshmem_module.aclshmem_create_tensor([M, 2*N], dtype=torch.float16, 
                                        mem_type=mem_type, device_id=rank)
        a = tensor[0:1, 0:N].fill_(9)
        b = tensor[0:1, N:2*N].fill_(0)
        torch.npu.synchronize()
        barrier.wait()
        nelems = M * N
        assert nelems % 32 == 0, "require 32-bit alignment"
        # 从一张新的卡上get数据到本pe，这里设成前一张卡
        newPe = (rank + npu_num - 1) % npu_num

        func = shmem_get(M, N, nelems, newPe)
        func(a, b)
        print("b after=", b)
        if torch.equal(a, b):
