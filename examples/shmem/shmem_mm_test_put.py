...
@tilelang.jit()
def shmem_put(M, N, nelems, newPe, dtype="int8"):
    @T.prim_func
    def main(
...
def worker(rank, barrier, rank_table_path):
    ...
    if result == 0:
        ...
        print("a before adress", a.data_ptr(), "rank", rank)
        print("b before adress", b.data_ptr(), "rank", rank)
        print("a before",a)
        print("b before",b)
        ...
        func = shmem_put(M, N, nelems, newPe)
        func(a, b)
        barrier.wait()
        print("a after adress", a.data_ptr(), "rank", rank)
        print("b after adress", b.data_ptr(), "rank", rank)
        print("a after",a)
        print("b after",b)
        if torch.equal(a, b):
