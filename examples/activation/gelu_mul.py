...
def gelu_mul(M, N, block_M, block_N, dtype="float16"):
    ...
    def main(
        ...
        with T.Kernel(m_num * n_num, is_npu=True) as (cid, vid):
            ...
            temp_ub2 = T.alloc_ub((block_M // VEC_NUM, block_N), dtype)
            temp_ub3 = T.alloc_ub((block_M // VEC_NUM, block_N), dtype)
            with T.Scope("V"):
                ...
                T.barrier_all()
                T.tile.fill(temp_ub2, 0.044715)
                T.barrier_all()
                T.tile.mul(temp_ub, temp_ub, temp_ub2)
                # T.tile.mul(temp_ub, temp_ub, 0.044715)
                T.barrier_all()
                T.tile.add(temp_ub, a_ub, temp_ub)
                T.barrier_all()
                T.tile.fill(temp_ub3, -1.5957691)
                T.barrier_all()
                T.tile.mul(temp_ub, temp_ub, temp_ub3)
                # T.tile.mul(temp_ub, temp_ub, -1.5957691)
                T.barrier_all()
                T.tile.exp(temp_ub, temp_ub)
                T.barrier_all()
                T.tile.fill(temp_ub2, 1.0)
                T.barrier_all()
                T.tile.add(temp_ub, temp_ub, temp_ub2)
