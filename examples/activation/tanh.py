...
def tanh(M, N, block_M, block_N, dtype="float"):
    ...
    def main(
        ...
        with T.Kernel(m_num * n_num, is_npu=True) as (cid, vid):
            ...
            nega_ub = T.alloc_ub((block_M // VEC_NUM, block_N), dtype)
            ...
            with T.Scope("V"):
                ...
                T.barrier_all()
                T.sub(nega_ub, zero_ub, a_ub)
                T.barrier_all()
                T.exp(a_ub, a_ub)
                T.exp(nega_ub, nega_ub)
                T.barrier_all()
                T.sub(b_ub, a_ub, nega_ub)
                T.add(denom_ub, a_ub, nega_ub)
                T.barrier_all()
                T.div(b_ub, b_ub, denom_ub)
                T.barrier_all()
