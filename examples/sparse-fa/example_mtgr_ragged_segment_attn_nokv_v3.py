...
@tilelang.jit(
    out_idx=[3],
    workspace_idx=[13, 14, 15],
    pass_configs=PASS_CONFIGS,
)
def mtgr_sparse_attn_nokv(
    heads,
    dim,
    kv_group=1,
    sm_scale=None,
    ...
    def main(
        ...
        tile_rules: T.Tensor([batch, max_q_tiles], "int32"),
        seg_first_tile: T.Tensor([batch, max_segs + 1], "int32"),
            ...
            for core_index in T.serial(my_iters):
                ...
                for k_i in T.serial(kv_iter_end):
                    ...
                    if k_i == s_local:
                        ...
                        if k_i >= seg_first_tile[b_i, seg_id]:
...
def mtgr_sparse_attn_wrapper(
    ...
    for b in range(B):
        ...
        splits, tile_seg_ids = compute_split_points(seg_lengths_list[b], block_M)
        seg_first = compute_seg_first_tile(tile_seg_ids, num_segs_b)
        t_rules = [rules[sid] for sid in tile_seg_ids]

        all_split_points.append(splits)
        all_tile_seg_id.append(tile_seg_ids)
        all_tile_rules.append(t_rules)
        all_seg_first_tile.append(seg_first)

        max_splits = max(max_splits, len(splits))
        max_q_tiles = max(max_q_tiles, len(tile_seg_ids))
    ...
    tile_seg_id_padded = []
    for ts in all_tile_seg_id:
        tile_seg_id_padded.append(ts + [-1] * (max_q_tiles - len(ts)))
    tile_seg_id_i32 = torch.tensor(tile_seg_id_padded, dtype=torch.int32)

    tile_rules_padded = []
    for tr in all_tile_rules:
        tile_rules_padded.append(tr + [-1] * (max_q_tiles - len(tr)))
    tile_rules_i32 = torch.tensor(tile_rules_padded, dtype=torch.int32)

    seg_first_tile_padded = []
    for sf in all_seg_first_tile:
        seg_first_tile_padded.append(sf + [0] * (max_segs + 1 - len(sf)))
    seg_first_tile_i32 = torch.tensor(seg_first_tile_padded, dtype=torch.int32)

    tiles_per_batch = [len(tr) for tr in all_tile_rules]
