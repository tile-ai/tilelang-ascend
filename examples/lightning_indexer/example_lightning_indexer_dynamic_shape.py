                    ...
                    for s2_id in T.serial(S2 // VECTOR_BASEN):
                        ...
                        if s2_id % merge_sort_times == merge_sort_times - 1:
                            if s2_id == merge_sort_times - 1:
                                T.tile.merge_sort(topk_global_ub2, topk_global_ub1, VECTOR_BASEN, merge_sort_times, 0)
                            else:
                                T.tile.merge_sort(mm_res_ub, topk_global_ub1, VECTOR_BASEN, merge_sort_times, 1)
                                T.barrier_all()
                                T.tile.topk(topk_global_ub2, topk_global_ub1, mm_res_ub, VECTOR_BASEN * merge_sort_times)
                        T.barrier_all()
                    T.barrier_all()
