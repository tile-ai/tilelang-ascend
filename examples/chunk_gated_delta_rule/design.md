...
pass_configs = {
    ...
    tilelang.PassConfigKey.TL_ASCEND_MEMORY_PLANNING: True,  # 内存复用
}
```
...
| 项 | 当前状态 | 潜在优化 |
|----|---------|---------|
| Workspace 大小 | 固定分配 | 动态分配（根据实际 NT） |
| 状态累积精度 | float32 → float16 | 可考虑保留 float32 输出 |
| 多 batch 并行 | N×H blocks | 可考虑 batch 内 head 并行 |
