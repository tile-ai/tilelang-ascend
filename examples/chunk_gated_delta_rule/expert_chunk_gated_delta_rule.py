...
def chunk_gated_delta_rule_fwd_h(
    ...
    g_c_t = g_flat.float().transpose(0, 1).contiguous() if USE_G else torch.empty((H, T_total), dtype=torch.float32, device=k.device)
    ...
    ker = chunk_gated_delta_rule_fwd_kernel(
        ...
        K,
        V,
        input_dtype=str(k.dtype).split(".")[-1],
        bt=BT,
        use_g=USE_G,
    ...
    slices = [h_out[i, : int(chunk_offsets_cpu[i + 1] - chunk_offsets_cpu[i])] for i in range(N)]
...
def test_chunk_gated_delta_rule(
    ...
    g = _chunk_local_cumsum_cpu(torch.randn((1, T_total, H), dtype=torch.float32) * GATE_SCALE, CHUNK_SIZE).npu() if use_g else None
