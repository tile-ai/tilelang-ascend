...
At present, the example operator in [`examples/flash_attention/flash_attn_bhsd_memory_planning`](../flash_attention/flash_attn_bhsd_memory_planning.py) is supported.
...
To test the packaged source code of the integrated TileLang-Ascend operator (i.e. `torch_tl_ascend.op_source.flash_attn_bhsd_memory_planning.flash_attention_fwd`):
...
```python
from torch_tl_ascend.op_source.flash_attn_bhsd_memory_planning import flash_attention_fwd
...
