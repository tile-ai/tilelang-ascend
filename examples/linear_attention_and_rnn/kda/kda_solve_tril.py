...
.. important::
   **Two implementations live here, and the pipeline always takes the second
   one.** ``kda_solve_tril()`` dispatches to the doubling Neumann series on the
   cube whenever the input is fp16 / bf16, which is every call stage 2 makes --
   it emits ``L`` in the data dtype, never fp32. The ``C >= 16`` clause in that
   dispatch is dead: the entry point already asserts ``C % 16 == 0`` for 32B row
   alignment. So the vector kernel documented immediately below runs only for an
   fp32 caller, or with ``KDA_SOLVE_CUBE=0``.

   That is not a reason to delete it, and calling it a "fallback" undersells
   what it is for. **The cube version is a TRUNCATED series** -- it covers
   ``L^7``, not the full ``L^(C-1)`` -- while the forward substitution below is
   exact. They are different algorithms computing the same thing, so the vector
   kernel is the only independent check on the truncation, and the truncation is
   a live risk: ``KDA_SOLVE_STEPS=1`` (covering ``L^3``) fails the keep gate in
   the pipeline at 8.260e-03 against a 5e-3 tolerance. ``_case_cube`` below
   therefore measures the cube result against BOTH the CPU golden and this
   kernel, holding ``KDA_SOLVE_CUBE=0`` down for the second one so that the
   control is not the subject.

   One operator, two algorithms, one file. The dispatch is four lines in
   ``kda_solve_tril()``.
...
def kda_solve_tril(Ltri, cu_seqlens=None):
    ...
    # bit-identical -- see kda_solve_tril_cube() below.
    #
    ...
    _cube = os.environ.get("KDA_SOLVE_CUBE", "1").lower() in ("1", "true", "yes", "on")
    if _cube and Ltri.dtype in (torch.float16, torch.bfloat16) and C >= 16:
...
def _case_cube(B, SEQ, H, HV, K, V, C, gate, dtype):
...
def main():
    ...
    ok = True
    print("== the cube path (the default), against the golden ==")
