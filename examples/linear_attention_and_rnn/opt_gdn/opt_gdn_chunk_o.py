    ...
    def main(
        ...
        with T.Kernel(B * H * chunk_num, is_npu=True) as (cid, vid):
            ...
            with T.Scope("V"):
                ...
                T.tile.mul(
                    coeff_ub, coeff_ub, msk_ub
                )  # This doesn't effect the result theoretically (because we apply the causal mask again later), but avoids overflow in exp in the next line
