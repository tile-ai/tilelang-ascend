...
def kernel(
    ...
    def main(
        ...
        final_state: T.Tensor([num_seqs, nv, dk, dv], dtype),
    ):
        with T.Kernel(NUM_CORES, is_npu=True) as (cid, vid):
            ...
            k_1d = T.alloc_ub([dk, 1], accum_dtype)  # [dk, 1]  broadcast src (k/q)
            v_f = T.alloc_ub([vec_block_v], accum_dtype)  # [vec_block_v]  V
            k_broadcasted_transpose = T.alloc_ub([vec_block_v, dk], accum_dtype)  # [dk, vec_block_v]
            k_broadcasted = T.alloc_ub([dk, vec_block_v], accum_dtype)  # [dk, vec_block_v]
            compute_buffer = T.alloc_ub([dk, vec_block_v], accum_dtype)  # [dk, vec_block_v] workspace

            # -- state matrix H ∈ R^{dk × vec_block_v} --
            h_vec = T.alloc_ub([dk, vec_block_v], accum_dtype)  # [dk, vec_block_v]  fp32 state
            h_load_vec = T.alloc_ub([dk, vec_block_v], dtype)  # [dk, vec_block_v]  load init_state
            h_store_vec = T.alloc_ub([dk, vec_block_v], dtype)  # [dk, vec_block_v]  store final_state

            # -- prediction & delta --
            pred_vec = T.alloc_ub([vec_block_v], accum_dtype)  # [vec_block_v]  H @ K
            pred_1d = T.alloc_ub([1, vec_block_v], accum_dtype)  # [1, vec_block_v]  reduce dst
            delta_vec = T.alloc_ub([vec_block_v], accum_dtype)  # [vec_block_v]  (V−pred)·gate
            delta_1d = T.alloc_ub([1, vec_block_v], accum_dtype)  # [1, vec_block_v]  broadcast src
            ...
            for work_i in T.serial(max_work_per_block):
                if work_i < count_work:
                    ...
                    T.tile.fill(h_vec, 0.0)  # H = 0  [dk, vec_block_v]
                    if state_idx >= 0:
                    ...
                    for t in T.serial(seq_len):
                        ...
                        T.reduce_sum(compute_buffer, pred_1d[0, :], dim=0)  # Σ_i H[i,v]·q̂[i]
