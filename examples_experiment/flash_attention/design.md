...
flash_attention/
├── design.md              # 本设计文档
└── example_gqa_bwd.py     # 完整实现（Forward + Backward + Golden Ref + Autograd + 测试入口）
```
...
| `example_gqa_bwd.py` | 单文件实现，包含全部 kernel + golden reference + autograd wrapper + main 入口 |
...
| `flashattn_fwd_v4` | Forward 优化版（L0 双缓冲 + Fixed Core + 批处理 + fine-grained sync） |
| `flashattn_bwd_preprocess` | Backward 预处理（Delta = sum(O × dO)） |
| `flashattn_bwd_postprocess` | Backward 后处理（dQ fp32→fp16） |
| `flashattn_bwd_pipeline` | Backward 主 kernel（5 GEMM + 批处理 + fine-grained sync） |
...
```bash
# 精度测试（L0）
python example_gqa_bwd.py --level l0 --batch 8 --h 32 --n_ctx 1024 \
    --d_head_qk 192 --d_head_v 128 --groups 16

# 性能基准
python example_gqa_bwd.py --level bench --batch 8 --h 32 --n_ctx 1024 \
    --d_head_qk 192 --d_head_v 128 --groups 16
```
