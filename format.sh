...
format_changed() {
    ...
    if git ls-remote --exit-code "$UPSTREAM_REPO" npuir-ci-dev &>/dev/null; then
        # First try to use the upstream repository directly
        MERGEBASE="$(git fetch "$UPSTREAM_REPO" npuir-ci-dev &>/dev/null && git merge-base FETCH_HEAD HEAD)"
    elif git show-ref --verify --quiet refs/remotes/origin/npuir-ci-dev; then
        # Fall back to origin/npuir-ci-dev if available
        BASE_BRANCH="origin/npuir-ci-dev"
        MERGEBASE="$(git merge-base $BASE_BRANCH HEAD)"
    else
        # Last resort, use local npuir-ci-dev
        BASE_BRANCH="npuir-ci-dev"
        MERGEBASE="$(git merge-base $BASE_BRANCH HEAD)"
    fi
...
spell_check_changed() {
    ...
    # exist on both branches.
    if git show-ref --verify --quiet refs/remotes/origin/npuir-ci-dev; then
        BASE_BRANCH="origin/npuir-ci-dev"
    else
        BASE_BRANCH="npuir-ci-dev"
    fi
