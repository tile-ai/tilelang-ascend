...
运行 `<skill-path>/scripts/run_examples.sh`，输出 tee 到日志文件。**`--project-root` 省略时默认为当前工作目录（cwd）。**

```bash
bash <skill-path>/scripts/run_examples.sh --backend <auto|ascendc|pto> [--project-root <cwd>] [--skip-aclgraph[=true|false]] [--skip-pytest[=true|false]] 2>&1 | tee <log_path>
```

示例（pto 后端，cwd 默认为项目根目录）：

```bash
bash <skill-path>/scripts/run_examples.sh --backend pto --skip-aclgraph --skip-pytest=false 2>&1 | tee run_pto.log
```
