...
Usage:
  python export_to_excel.py --log <log_file> --backend <auto|ascendc|pto>
...
timeout_fill = PatternFill(start_color="FFD966", end_color="FFD966", fill_type="solid")
...
def classify_failure(last_lines):
    ...
    NOTE: 本函数与 tilelang-pr-verify/scripts/generate_report.py 中 classify_failure 保持同步；
    修改失败分类规则时两处必须同步更新。
    """
    text = "\n".join(last_lines)
...
def parse_log(log_path):
    ...
    NOTE: 本函数与 tilelang-pr-verify/scripts/generate_report.py 中 parse_log 保持同步；
    解析逻辑修改时两处必须同步更新。
    """
    results = []
    ...
    for match in pattern.finditer(content):
        ...
        results.append((script, status, fail_type, fail_detail, start_time, end_time, elapsed))
...
def write_round_sheet(wb, sheet_name, results):
    ...
    headers = ["序号", "测试脚本", "结果", "开始时间", "结束时间", "耗时", "失败类型", "失败详情"]
    ...
    for i, (script, status, fail_type, fail_detail, start_time, end_time, elapsed) in enumerate(results, 1):
        row = i + 1
        ...
        if status == "PASSED":
            ...
            cell_status.fill = timeout_fill
            cell_status.font = Font(color="7F6000", bold=True)
        else:
            cell_status.fill = fail_fill
            cell_status.font = Font(color="9C0006")
        ws.cell(row=row, column=4, value=start_time or None).border = thin_border
        ws.cell(row=row, column=5, value=end_time or None).border = thin_border
        ws.cell(row=row, column=6, value=elapsed or None).border = thin_border
        ws.cell(row=row, column=7, value=fail_type or None).border = thin_border
        ws.cell(row=row, column=8, value=fail_detail or None).border = thin_border
    ...
    failed = sum(1 for r in results if r[1] in ("FAILED", "TIMEOUT"))
...
def write_failure_summary_sheet(wb, results):
    ...
    fail_types = {}
    for script, status, fail_type, fail_detail, *_ in results:
        if status in ("FAILED", "TIMEOUT"):
...
def write_comparison_sheet(wb, round_sheets_info):
    ...
    all_scripts = sorted(set(s for _, results_dict in round_sheets_info for s in results_dict.keys()))
    ...
    for i, script in enumerate(all_scripts, 1):
        ...
        if num_rounds >= 2:
            ...
            # Treat TIMEOUT and FAILED equivalently as "not passed" for the
            # FIXED / NEW FAIL semantics (regression detection).
            def _norm(s):
                return "FAILED" if s in ("FAILED", "TIMEOUT") else s

            change = "无变化"
            if _norm(prev_status) != _norm(curr_status):
                if _norm(prev_status) == "FAILED" and _norm(curr_status) == "PASSED":
                    change = "FIXED"
                elif _norm(prev_status) == "PASSED" and _norm(curr_status) == "FAILED":
                    change = "NEW FAIL"
                else:
                    change = "变化"
    ...
    ws.merge_cells(start_row=summary_row, start_column=1, end_row=summary_row, end_column=3 + num_rounds)
    ...
    for _, results_dict in round_sheets_info:
        ...
        f = sum(1 for v in results_dict.values() if v in ("FAILED", "TIMEOUT"))
        passed_counts.append(p)
        failed_counts.append(f)

    stats.append(("总数", [len(all_scripts)] * num_rounds, [0] if num_rounds >= 2 else None))
    stats.append(("通过", passed_counts, [passed_counts[-1] - passed_counts[-2]] if num_rounds >= 2 else None))
    stats.append(("失败", failed_counts, [failed_counts[-1] - failed_counts[-2]] if num_rounds >= 2 else None))
    ...
    if num_rounds >= 2:
        # FIXED / NEW FAIL use normalized status (TIMEOUT counts as FAILED).
        def _norm_s(s):
            return "FAILED" if s in ("FAILED", "TIMEOUT") else s
        ...
        fixed = sum(
            ...
            if _norm_s(round_sheets_info[-2][1].get(s)) == "FAILED" and _norm_s(round_sheets_info[-1][1].get(s)) == "PASSED"
        )
        new_fail = sum(
            1
            for s in all_scripts
            if _norm_s(round_sheets_info[-2][1].get(s)) == "PASSED" and _norm_s(round_sheets_info[-1][1].get(s)) == "FAILED"
        )
...
def export_to_excel(log_path, backend, excel_path):
    ...
    print(f"Sheet '{sheet_name}': {passed} passed, {failed} failed, pass rate {passed / len(results) * 100:.0f}%")
    if len(all_round_data) >= 2:
        prev_results = all_round_data[-2][1]

        # Treat TIMEOUT as FAILED for FIXED / NEW FAIL console reporting.
        def _ns(s):
            return "FAILED" if s in ("FAILED", "TIMEOUT") else s

        fixed = sum(1 for s in results_dict if _ns(prev_results.get(s)) == "FAILED" and _ns(results_dict[s]) == "PASSED")
        new_fail = sum(1 for s in results_dict if _ns(prev_results.get(s)) == "PASSED" and _ns(results_dict[s]) == "FAILED")
        print(f"Comparison vs {all_round_data[-2][0]}: FIXED {fixed}, NEW FAIL {new_fail}, Still failing {failed - new_fail}")
    print(f"Total sheets: {wb.sheetnames}")


def main():
    parser = argparse.ArgumentParser(description="Export TileLang-Ascend test results to Excel")
    parser.add_argument("--log", required=True, help="Path to run_examples.sh output log file")
    parser.add_argument(
        "--backend", required=True, choices=["auto", "ascendc", "pto"], help="Backend type used in the test run (auto, ascendc or pto)"
    )
    parser.add_argument("--output-dir", default="./tmp", help="Output directory for the Excel file (default: ./tmp)")
    args = parser.parse_args()
    os.makedirs(args.output_dir, exist_ok=True)
    excel_path = os.path.join(args.output_dir, "run_examples_results.xlsx")
    export_to_excel(args.log, args.backend, excel_path)
