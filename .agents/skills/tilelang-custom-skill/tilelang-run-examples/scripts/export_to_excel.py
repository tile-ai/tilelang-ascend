...
def write_comparison_sheet(wb, round_sheets_info):
    ...
    if num_rounds >= 2:
        fixed = sum(1 for s in all_scripts if round_sheets_info[-2][1].get(s) == "FAILED" and round_sheets_info[-1][1].get(s) == "PASSED")
        new_fail = sum(
            1 for s in all_scripts if round_sheets_info[-2][1].get(s) == "PASSED" and round_sheets_info[-1][1].get(s) == "FAILED"
        )
...
def export_to_excel(log_path, backend, excel_path):
    ...
    if len(all_round_data) >= 2:
        ...
        fixed = sum(1 for s in results_dict if prev_results.get(s) == "FAILED" and results_dict[s] == "PASSED")
        new_fail = sum(1 for s in results_dict if prev_results.get(s) == "PASSED" and results_dict[s] == "FAILED")
...
def main():
    ...
    excel_path = os.path.join(os.getcwd(), "run_examples_results.xlsx")
