...
> - `tilelang/language/ascend.py`、`tilelang/language/ascend_tile.py`
> - `examples/` 目录下的实际算子实现
> - `testing/python/language/` 目录下的测试用例
...
| **Softmax/LayerNorm** | [api-compute](references/api-compute.md) | T.reduce_max/sum（需 tmp buffer）、T.tile.exp/sub/div |
...
| `T.reduce_sum/max/min(buffer, out, tmp, dim)` | 按维度归约 |
