...
description: TileLang-Ascend 全量运行 examples 目录下算子脚本的技能。支持选择后端类型（auto/ascendc/pto）、是否运行 aclgraph 脚本、并行控制等。触发关键词："全量运行"、"run all examples"、"跑全部算子"、"运行所有示例"、"批量测试"、"全量测试"、"run examples"、"bench all"、"跑 examples"。
---
...
其中 `<skill-path>` 为本 skill 的目录路径。脚本自动从自身位置推导项目根目录（`.agents/skills/tilelang-run-examples/scripts/` → 上溯 4 层到项目根），也可通过 `--project-root` 显式指定。
...
| `--skip-aclgraph[=true|false]` | 跳过 `aclgraph/` 目录下的脚本（算力平台不支持，运行会导致环境崩溃）。`--skip-aclgraph` 或 `--skip-aclgraph=true` 跳过；`--skip-aclgraph=false` 不跳过 | 跳过（默认 true） |
| `--skip-pytest[=true|false]` | 跳过 pytest 测试阶段。`--skip-pytest` 或 `--skip-pytest=true` 跳过；`--skip-pytest=false` 启用 | 跳过（默认 true） |
| `--dirs <dirs...>` | 只运行指定目录（增量模式） | 全量运行 |
| `--max-jobs N` | 最大并行任务数 | 8 |
| `--project-root <path>` | 项目根目录路径 | 自动从脚本位置推导（上溯 4 层） |
...
该补丁覆盖所有 Python 调用方式：直接 `python script.py`、Shell 脚本内嵌 Python 调用（如 `gemm_aot/run_example_gemm_aot.sh`）、以及 pytest 运行。
...
bash .agents/skills/tilelang-run-examples/scripts/run_examples.sh

# pto 后端（通过环境变量覆盖，不修改源文件）
bash .agents/skills/tilelang-run-examples/scripts/run_examples.sh --backend pto

# 不跳过 aclgraph（注意：算力平台不支持，运行可能导致环境崩溃）
bash .agents/skills/tilelang-run-examples/scripts/run_examples.sh --backend pto --skip-aclgraph=false

# 只运行指定目录
bash .agents/skills/tilelang-run-examples/scripts/run_examples.sh --dirs "gemm softmax normalization"

# 控制并行度
bash .agents/skills/tilelang-run-examples/scripts/run_examples.sh --max-jobs 4

# 启用 pytest（默认跳过，加 --skip-pytest=false 启用）
bash .agents/skills/tilelang-run-examples/scripts/run_examples.sh --skip-pytest=false

# 手动设置环境变量（不通过脚本，直接运行单个示例）
# 必须同时设置 PYTHONPATH 以加载 usercustomize.py 补丁
export TILELANG_JIT_TARGET=pto
export PYTHONPATH=.agents/skills/tilelang-run-examples/scripts${PYTHONPATH:+:$PYTHONPATH}
...
**以下 5 个步骤必须严格按顺序执行，每一步都必须与用户交互确认后才进入下一步。禁止跳过任何询问步骤直接执行。**

### 步骤 1：询问用户配置（必须使用 question 工具）

- **1a** 询问后端类型（auto、ascendc 或 pto，默认 auto）
- **1b** 询问是否跳过 aclgraph（默认跳过，算力平台不支持运行会导致环境崩溃；可选择 `--skip-aclgraph=false` 运行）

**必须使用 `question` 工具向用户提问，不能凭假设直接跳过。**
...
```bash
bash .agents/skills/tilelang-run-examples/scripts/run_examples.sh --backend <auto|ascendc|pto> [--skip-aclgraph[=true|false]] [...] 2>&1 | tee <log_path>
```
...
```bash
python <skill-path>/scripts/export_to_excel.py --log <log_path> --backend <auto|ascendc|pto> [--excel <output_path>]
```
...
| `--excel` | Excel 输出文件路径 | `test_results.xlsx`（当前目录） |
...
| 步骤 | 检查项 | 是否与用户交互 |
|------|--------|--------------|
| 1 | 是否用 question 工具询问了后端类型和 aclgraph？ | ✅ 必须 |
