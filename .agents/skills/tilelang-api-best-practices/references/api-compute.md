...
> **⚠️ 内部精度限制（`T.tile.exp` / `T.tile.ln` / `T.tile.reciprocal`）**
>
> 这三个 API 的内部计算精度行为在 **float32 输入 + 大值域（\|x\|>16）** 场景下可能不达标（MARE≈0.9）。已知现象：
> - **fp16/bf16 路径**（因 cast 到 fp32 中间计算）：精度好（MERE 全 0）✓
> - **float32 路径**（直接用 fp32 buffer 调 `T.tile.exp`）：精度差（MARE≈0.908）❌
>
> **根因（疑似）**：`T.tile.exp/ln` 内部可能以 fp16 计算无论 buffer dtype（与 `T.tile.sigmoid` 行为不同）。
>
> **验证方法**：用最小 reproducer（单元素 buffer）验证 API 在目标 dtype 下的精度行为：
> ```python
> # 验证 T.tile.exp 在 fp32 下的精度
> @tilelang.jit
> def test_exp(M, N, dtype="float32"):
>     @T.prim_func
...
>             T.copy(a, B)
>     return main
> # 对比 torch.exp(x) for x in [-20, -50, -88]
> ```
>
> **替代方案**（精度不达标时）：改用 `T.Parallel + T.exp/T.log` 符号 API（preserves buffer dtype，生成 vectorized `AscendC::Exp/Ln`）：
> ```python
> for i, j in T.Parallel(block_M, block_N):
>     tmp[i, j] = T.exp(x_fp32[i, j])  # 符号 API，保持 fp32 精度
> ```
>
> **注意**：`T.tile.sigmoid` 的精度行为可能不同于 exp/ln——mish 用 `2*sigmoid(2s)-1` 实现 tanh 在 fp32 下未触发精度问题（官方 cann-bench case 13/15/18 float32 全 PASS）。如需 tanh 实现，优先用 sigmoid 等价而非 `T.tile.exp/ln` 组合。
>
> **参考案例**：`custom/mish/mish.py` L91 `T.tile.exp` + 官方 cann-bench case 5/11/20 float32 失败（`custom/mish/perf_tuning/cannbench_official_iter3.json`）。
