...
| ⚠️ `T.tile.compare(mask, x, threshold, "GT"/"GE"/"LT"/"LE"/"EQ"/"NE")` | **已知 bug API**：128 元素 buffer 时后 64 位 mask 反转，详见 [troubleshooting.md](references/troubleshooting.md) |
| ⚠️ `T.tile.select(dst, mask, src0, src1)` | **已知 bug API**：与 compare 配合时受 mask 损坏影响，详见 [troubleshooting.md](references/troubleshooting.md) |

> ⚠️ **`T.tile.compare` + `T.tile.select` 已知 bug 警告**：
> 当 buffer 元素数为 128 的整数倍时，`compare` 生成的 mask 后 64 位被反转，导致 `select` 选择错误 source。
> **规避方法**：优先用数值稳定公式替代条件分支（如 `tanh(x)=1-2/(exp(2x)+1)` 替代 `(e^x-e^-x)/(e^x+e^-x)`），完全消除 compare/select 依赖。
> 完整故障排查见 [references/troubleshooting.md](references/troubleshooting.md)。
