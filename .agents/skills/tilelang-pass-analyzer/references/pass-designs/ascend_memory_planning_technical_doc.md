...
| 特性 | 说明 |
|-----|-----|
| 内存域 | shared.ub, shared.l1, wmma.matrix_a/b/accumulator |
...
|-------|-----------|----------|---------|
| shared.ub | 196352 | UB 统一缓冲 | alloc_shared(推断为UB), copy_gm_to_ub |
| shared.l1 | 524032 | L1 | alloc_shared(推断为L1), copy_gm_to_l1 |
...
```
输入 IR（简化）：
  Allocate(bufA, shared.ub, 1024)
  Allocate(bufB, shared.ub, 2048)
  Allocate(bufC, shared.ub, 512)
  // 三者都在同一作用域，从头用到尾
...
**输入 IR**：
```
Allocate(bufA, shared.ub, 1000)  // size 非32倍数
Allocate(bufB, shared.ub, 512)
```
...
```
ASCEND_SHARED_MEM_UB_SIZE        = 196352   // shared.ub
ASCEND_SHARED_MEM_L1_SIZE        = 524032   // shared.l1
