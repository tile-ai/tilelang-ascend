...
2. **L1 / UB 区分**：根据 buffer 是否参与 Cube（GEMM）或 Vector（逐元素）计算，把 `shared` 改写为 `shared.l1`（L1）或 `shared.ub`（UB）。
...
处理流程:
  ...
  for handle, alloc in alloc_info:
    ...
    for handle, alloc in alloc_info:
      ...
      if qualified: alloc.corrected_scope = "shared.l1"   # 保留 L1
...
```tir
allocate(A_l1, "float16", "shared.l1", [M, K])         ← 保留 L1
allocate(B_l1, "float16", "shared.l1", [K, N])         ← 保留 L1
...
call_extern("gemm_v0",
            access_ptr(A_l1[shared.l1]),    # 仍为 L1（gemm 输入语义）
            access_ptr(B_l1[shared.l1]),    # 仍为 L1
...
* `A_l1` / `B_l1` 用作 `gemm_v0` 输入，`used_in_cube=true`，保留 `shared.l1` (L1)。
...
| `shared` | `shared.l1` | L1，Cube 侧片上缓存 |
