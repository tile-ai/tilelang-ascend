...
PrimFunc {
  ...
  params: [
    ...
    Var("B", Pointer(float16, "shared.ub")),
  ...
  body: Stmt {
    SeqStmt {
      Allocate {
        buffer_var: Var("B", Pointer(float16, "shared.ub"))
        extents: [1024]
        condition: True
        body: Stmt {
          For {
...
Buffer A[scope, dtype, shape]
  - scope: global / shared.ub / shared.l1 / L1 / UB / L0A / L0B / L0C
...
Phase 1: AscendMemoryPlanner (StmtExprVisitor)
  ...
  VisitStmt_(AllocateNode):
    1. 记录 buffer scope (shared.l1/wmma.matrix_a/wmma.matrix_b/wmma.accumulator)
    2. 计算 buffer size
