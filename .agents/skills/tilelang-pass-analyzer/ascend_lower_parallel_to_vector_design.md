...
| 维度 | 说明 |
|------|------|
| 所属阶段 | Phase 2: OptimizeForTarget（后端优化） |
| 执行时机 | 在 `ascend_sync_insert` 之前 |
| 平台特性 | Ascend 专用 Pass |
| 启用方式 | 默认启用，无配置开关 |
...
```
LowerTileOp → AscendLowerParallelToVector → AscendMemoryPlanning → AscendSyncInsert
       │                  │                        │                    │
       │                  │                        │                    │
   Tile操作Lowering    Parallel向量化        内存地址规划           同步点插入
```