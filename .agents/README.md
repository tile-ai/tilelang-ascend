...
OpenCode 会自动加载项目规范，触发环境预检，编排 4 阶段流程。**唯一需要你确认的两件事**：编程模式（Developer / Expert / 混合）和精度通过后是否需要性能调优。
...
Orchestrator 会自动驱动 4 阶段状态机：
**环境预检 → Stage 1 设计 → Stage 2 实现 → (Stage 3 精度修复) → (Stage 4 性能调优)**
...
| 代理 | 模式 | 职责 |
|:---|:---|:---|
| [tilelang-op-orchestrator](../.opencode/agents/tilelang-op-orchestrator.md) | Primary | 4 阶段状态机、工件门禁、Subagent 调度、设计回退控制、环境预检 |
| [tilelang-op-analyst](../.opencode/agents/tilelang-op-analyst.md) | Subagent | Stage 1 算子设计（含需求理解、设计回退） |
| [tilelang-op-developer](../.opencode/agents/tilelang-op-developer.md) | Subagent | Stage 2 实现 + Stage 3 精度修复（含 `[DESIGN_ERROR]` 识别） |
| [tilelang-op-perf-tuner](../.opencode/agents/tilelang-op-perf-tuner.md) | Subagent | Stage 4 性能调优（可选） |
...
Stage 1: 算子设计 → @tilelang-op-analyst
    ↓                                 产物：DESIGN.md
Stage 2: 代码实现 → @tilelang-op-developer
    ├─ PRECISION_FAIL ─→ Stage 3: 精度修复 → @tilelang-op-developer
    │                                 产物：history_version/ 备份 + 修复后 impl
    └─ PRECISION_PASS ─┐
                       ↓
            ┌── 询问用户是否需要性能调优 ──┐
            │                              │
            ↓ 不需要                       ↓ 需要 + 提供调优信息
         SUCCESS                  Stage 4: 性能调优 → @tilelang-op-perf-tuner
                                          产物：perf_tuning/
                                          ↓
                                      SUCCESS
```
...
> **关键特性**：
> - **Stage 4 是可选阶段**，由用户在精度通过后主动确认
> - **设计回退机制**：Developer 在 Stage 2/3 中可标记 `[DESIGN_ERROR]` 触发回退到 Stage 1 重新设计（不设次数上限）
> - **环境预检**：一次性通过即可，续跑不重复
...
│   └── {op}_impl_s3_attempt{N}.py         # 精度修复备份
...
- **完整算子开发**：使用 `tilelang-op-orchestrator` agent，自动驱动 4 阶段
...
<details>
<summary><b>Stage 4 性能调优为什么是可选的？</b></summary>
...
- 你回 "需要" → 继续询问性能调优必要信息（目标类型、目标数值、baseline、shape、噪声阈值、迭代上限），追加写入 DESIGN.md → 进入 Stage 4
...
Developer 在 Stage 2 / Stage 3 中识别到这些情形时，会在输出加 `[DESIGN_ERROR]` 标记。Orchestrator 据此：
