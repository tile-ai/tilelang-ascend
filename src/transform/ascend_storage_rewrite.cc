...
private:
  ...
  // "shared", empty tag) and "static shared memory" to UB buffers (scope
...
private:
  ...
  bool IsSpecialTaggedMemory(const StorageScope &scope) {
    return scope.tag.length() != 0 && scope.tag != ".ub" &&
...
public:
  ...
  PrimExpr VisitExpr_(const BufferLoadNode *op) final {
    ...
    if (node.same_as(modified)) {
      ...
      BufferLoad legalized_modified(modified->buffer, modified->indices,
                                    modified->span);
      if (shuffle_index >= 0) {
        return Shuffle::ExtractElement(std::move(legalized_modified),
                                       shuffle_index);
      }
...
Pass AscendStorageRewrite(bool is_npu = false) {
  auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
    ...
    PrimFunc result = PointerValueTypeRewrite(std::move(f), true, false, false,
...
TVM_REGISTER_GLOBAL("tl.transform.AscendStorageRewrite")
    .set_body_typed(AscendStorageRewrite);
