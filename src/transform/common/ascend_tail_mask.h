 ...
 *    be dynamic (a `Select(...)` over the block index), which is exactly why the
 *    rewrite produces a *runtime* tail helper instead of a compile-time shape.
 *  - `physical_row` / `physical_col` are the allocated UB tile dims; `physical_col`
 *    is the row pitch used to derive repeat strides.
