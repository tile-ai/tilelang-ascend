...
extern "C" int tilelang_dump_from_host_args(const void *argsBuf,
    ...
    if (paramInfo->count == 0 ||
        paramInfo->count > TILE_LANG_MAX_TENSOR_COUNT) {
        return -3;
    }
    ...
    if (acldumpSaveExceptionInfo != nullptr) {
        ...
        char filePath[512];
        snprintf(filePath, sizeof(filePath), "%s/tilelang_exception_dump_%d_%ld.log",
                 dumpDir, (int)getpid(), (long)now);

        FILE *fp = fopen(filePath, "w");
        if (fp == nullptr) {
            snprintf(filePath, sizeof(filePath),
                     "/tmp/tilelang_exception_dump_%d_%ld.log",
                     (int)getpid(), (long)now);
            fp = fopen(filePath, "w");
        }
        if (fp != nullptr) {
            ...
            for (size_t t = 0; t < count; t++) {
                ...
                fprintf(fp, "tensor[%zu]: addr=0x%lx, size=%zu bytes, dataType=%d\n",
                        t, (unsigned long)devAddr,
                        tensorInfos[t].tensorSize,
                        tensorInfos[t].dataType);
                if (devAddr != 0 && tensorInfos[t].tensorSize > 0) {
                    size_t dumpBytes = tensorInfos[t].tensorSize;
                    if (dumpBytes > 128) dumpBytes = 128;
                    uint8_t *hostData = (uint8_t *)malloc(dumpBytes);
                    if (hostData != nullptr) {
                        aclError cpRet = aclrtMemcpy(hostData, dumpBytes,
                                                      (void *)devAddr, dumpBytes,
