...
void CodeGenTileLangAscend::VisitStmt_(const AllocateNode *op) {
  ...
  auto print_buffer =
      [&](const std::string &pos) {
        ...
        local_tensor_dtypes_[buffer] = op->dtype;
...
CodeGenTileLangAscend::PrintBufferOffset(const CallNode *call_arg_node,
  ...
  auto alloc_dtype = local_tensor_dtypes_.find(_var);
  DataType access_dtype = GetAccessPtrDtype(call_arg_node);
  if (alloc_dtype != local_tensor_dtypes_.end() &&
      alloc_dtype->second != access_dtype) {
...
void CodeGenTileLangAscend::TailBroadcastOpCodegen(const CallNode *op) {
  // Original 2D broadcast args(0..8), followed by output/input valid rects.
  ICHECK_EQ(op->args.size(), 13U);
  int axis = is_one(op->args[8]) ? 1 : 0;
  if (axis == 1) {
    ...
    this->PrintIndent();
    this->stream << "tl::ascend::Broadcast<" << dtype << ", 2, 1, false>("
                 << dst << ", " << src << ", " << tmp << ", (uint32_t[]){"
                 << PrintExpr(op->args[5]) << ", " << PrintExpr(op->args[6])
                 << "}, (uint32_t[]){" << PrintExpr(op->args[7]) << ", "
                 << PrintExpr(op->args[8]) << "});\n";
    return;
  }
  std::string dtype = getType(GetAccessPtrDtype(op->args[1].as<CallNode>()));
  std::string dst = PrintBufferOffset(op->args[1].as<CallNode>());
  std::string src = PrintBufferOffset(op->args[2].as<CallNode>());
  std::string vrow = PrintExpr(op->args[9]);
  std::string vcol = PrintExpr(op->args[10]);
  std::string src_vrow = PrintExpr(op->args[11]);
  std::string src_vcol = PrintExpr(op->args[12]);
  std::string dst_pcol = PrintExpr(op->args[6]);
  std::string src_pcol = PrintExpr(op->args[8]);
  this->PrintIndent();
