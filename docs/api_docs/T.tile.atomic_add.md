...
将本地 tensor（UB/L0C）的数据原子累加到 GM 目标 tensor：`dst[GM][i] += src[local][i]`
...
> **类型说明**：
> - **tensor**：通过 `T.alloc_ub`、`T.alloc_shared`、`T.alloc_L0C` 等分配的缓冲区（Buffer），或其切片（BufferRegion）
> - **global scope**：通过 `T.alloc_gm` 或外部传入的 GM buffer
> - **local scope**：UB、L0C 等非 GM 内存区域
...
2. src 的 buffer scope 必须为 local（UB/L0C 等），不能为 `global`，否则抛出 `ValueError`
3. dst 与 src 的 dtype 必须相同（所有路径）
4. 仅支持 local（VECOUT/L0C）→ GM 方向的 DMA 搬运附带原子累加（硬件约束）
...
T.tile.atomic_add(C, src_ub)  # C 为外部传入的 GM tensor
```
...
T.tile.atomic_add(C, C_L0)  # C 为外部传入的 GM tensor
```
