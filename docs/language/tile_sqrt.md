...
对源操作数逐元素开平方根：`dst[i] = sqrt(src0[i])`
...
2. dst 可与 src0 为同一 buffer（原地运算，如 `T.tile.sqrt(a_ub, a_ub)`）
3. 操作数地址需 32 字节对齐（硬件约束）
4. 精度：真机实测 float32 最大相对误差 ≤ 1e-5，float16 ≤ 1e-3（rtol/atol 口径，与测试容差一致）
5. 特殊值遵循 IEEE 语义：`sqrt(0)=0`、`sqrt(负数)=nan`、`sqrt(inf)=inf`、`sqrt(nan)=nan`
...
T.tile.sqrt(dst, src0)  # dst[i] = sqrt(src0[i])
```