...
2. dst 可与 src0 为同一 buffer（原地运算，如 `T.tile.abs(a_ub, a_ub)`）
3. 操作数地址需 32 字节对齐（硬件约束）
4. 精度：逐位精确（真机实测 float16/float32 结果与 |x| 的位模式完全一致）
5. 特殊值遵循 IEEE 语义：`abs(-inf)=inf`、`abs(nan)=nan`、`abs(0)=0`
