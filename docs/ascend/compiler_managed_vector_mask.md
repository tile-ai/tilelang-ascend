...
> **Temporary `uint8`/`float32` And/Or compatibility:** these types retain the pre-existing
> count-form call with unchanged count. On dav-c220 it executes 16-bit lanes: count `N` accesses
> `2N` bytes, twice the logical `uint8` extent or half the `float32` extent. Existing examples
> store packed Compare predicates in oversized `float32` buffers, so their valid mask bits are
> covered; this does not establish general `float32` bitwise support. New code must not add
> these unsupported calls. Managed raw lowering still accepts only `int16`/`uint16`. The
> compatibility path remains isolated until typed views can express the physical type and count,
> with byte pairing, alignment, and odd-byte tails handled explicitly.
