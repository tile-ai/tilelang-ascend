...
TileLang是一种面向tile级别编程的DSL，基于其tile级的数据块抽象，使开发者能够以一种高级、可组合的方式表达和优化底层内核行为。TileLang可以让开发者显式控制内存分配、数据移动、布局和并行执行。

下图展示了TileLang程序如何从高级描述逐步降低为面向特定硬件的可执行程序。我们提供了三种不同的编程接口，分别面向初学者、开发者和专家用户，这些接口分别位于这一降低流程的不同层级。Tile语言还允许在同一内核中混合使用这些接口，使用户能够根据自身需求选择最适合的抽象层次进行编程。
...
我们假设您已搭建好包含CANN（至少8.3.RC1版本）和torch-npu（至少2.6.0.RC1版本）的升昇腾环境，首先设置CANN环境变量。
...
下面是一个TileLang kernel开发的两个Tile element-wise add的示例：
...
接下来是算子kernel函数的定义和实现，TileLang算子kernel函数由`@T.prim_func` 装饰器在模块内定义（`add_kernel`），这部分会被TileLang编译器编译成可以在NPU上运行的二进制代码。Python函数`tile_add`是kernel的wrapper，通过`return add_kernel`语句返回指向`add_kernel`的函数对象，供后续调用。
...
`T.Tensor`声明数据类型Tensor数据缓冲区，并指定其形状和数据类型，A/B/C是kernel的参数，其中通过`@jit`注解的参数out_idx=[-1]来指定最后一个参数、也即C为输出参数，其他未输入参数（关于`@jit`后续会详细说明，可以理解为调用其注解的函数时候会触发即时编译）。
...
这个切分的Tile的数量为`m_num * n_num`，上述的`T.kernel`可以理解为为切分后的每个Tile创建一个并发执行单元，kernel里定义的操作在每个Tile上执行搬运、计算，多个并发执行单元并发运行，互不影响。`with T.kernel`返回的元组（cid, vid）中，cid可以理解为计算任务的数量（每个Tile对应一个计算任务，对应一个逻辑核来处理），vid范围为0-1，即每个Tile再由两个子计算单元来完成。



示例代码中的其余部分是通过调用torch来实现ref值，然后和算子计算输出进行比对以验证算子逻辑的正确性。
...
- 运行阶段
  1. 库文件加载：通过torch_npu运行时库将.so文件加载到Python环境中
...
在TileLang kernel开发中，通过**@jit**注解来触发调用时的即时编译。
