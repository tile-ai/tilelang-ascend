...
> 本文只讲 `compare`。整体尾块框架、`GM -> UB` 有效区识别和 #1360 的基础机制可先参考本机
> 模板 `/mnt/workspace/tilelang-ascend/docs/tail.md`。与本文配套的另外两类场景是
> [`select`](../tail_select/README.md) 和 [`broadcast`](../tail_broadcast/README.md)。
...
不会强行改写，而是保留既有 native compare 路径。大多数受检失败分支会清除传播状态；当前
`BufferLoad` scalar 的参数数量分支存在提前返回、未清旧状态的缺口，详见第 11 节。
...
dynamic mask width、select 的 `valid_col` 和尾块 store clamp 排除，不属于逻辑输出。
...
这里刻意保留 `valid_col` 为“逻辑元素列数”，没有把它改成 byte 数。原因是下游 select 需要知道
要产生多少个数据元素，而不是要读多少个 predicate byte。只有 PTO 创建 mask Tile view 时，才
临时计算 `(valid_col + 7) / 8`。
...
默认构造的 `TailMaskInfo` 是未跟踪的 `kFull` 状态。这样下游 select 不会误以为 native compare
生成的是本框架已经证明过布局的 packed mask。
...
此时还没有取得 `dst_v`，所以 7 参数 `BufferLoad` scalar 会直接保留 native，却不会清理同一个
mask UB 可能残留的旧 `kPackedCmp` 状态。这是当前实现缺口，而不是设计目标；若 mask buffer 被
复用，后续 select 存在误用旧 provenance 的风险。修复时应先解析 dst，再在 arity 不支持时清
state，并增加“同一个 mask UB 先成功 tail compare、再被 BufferLoad compare 覆写”的回归测试。
...
barrier，标量访问可能观察到尚未完成的结果。清理后再做一次 barrier，保证后续 select 不会早于
清理操作消费 mask。
...
控制 mask，它改变的是 shape 和同一输入值的复制关系。
...
设计原则是“保留 native”与“继续传播 packed 状态”不能同时发生。当前主要守卫失败会清掉 dst
state，确保整个 compare-select 链统一回退；但前述 7 参数 `BufferLoad` 提前返回尚未满足该原则，
必须作为已知 stale-state 风险处理，不能在验收结论中写成“所有 fallback 均已清状态”。
...
- 2 backend × 2 dtype × 2 compare 类型 × 2 select 类型；
...
- full tile 保留 native；
- flag off 保留 native；
- fp32 `block_N=128` 的超宽场景回退并清状态；
- 外部 GM uint8 mask 不能伪装成 tracked compare mask。

## 12.2 设备数值测试

`_pack_compare_mask` 是 CPU golden：
...
这与硬件布局的 `bit_index = c % 8` 完全一致。测试不仅验证 select 输出，还把 `mask_ub` 回写
GM，与 golden 逐 byte、`rtol=0, atol=0` 对比，因此能检测最后一个 byte 高位没有清零的问题。

Developer 用例覆盖 tensor/tensor 和 scalar/scalar 组合、fp16/fp32、双后端；Expert 用例使用
`T.Scope("V")` 和手工 barrier，证明尾块 pass 不要求必须开启自动同步。
...
7. 下游 select 读取同一个 `kPackedCmp` 状态并只产生 `[1,5]` 的有效数据。

# 14. 当前限制与后续扩展点

1. 多 vector repeat 的 compare 还未开放；需要验证跨 repeat packed 输出的 byte 起点和行距。
2. BufferLoad scalar 仍走 native；当前提前返回还需先修复旧 `kPackedCmp` 状态未清理的问题。若要
   真正支持 tail，内部 ABI 还需要携带 scalar buffer、index 和必要 barrier。
...
除已经明确列出的 BufferLoad stale-state 缺口外，其余限制通过守卫和 fallback 表达。不应通过
放宽 `ICHECK` 或删除状态清理来绕过；应先补状态语义和对应回归测试。
