...
1. **只处理实现与精度修复**
   - Stage 2 负责生成单一交付文件 `example_{op}.py`（含 `@tilelang.jit` kernel + 内嵌 PyTorch golden + 几个常用 shape 的 test 用例 + main 块），并完成首跑判定。
...
当 Orchestrator 指定执行 Stage 2 时，你负责根据 `DESIGN.md` 生成**单一交付文件** `example_{op}.py`，包含 `@tilelang.jit` kernel、内嵌 PyTorch golden、几个常用 shape 的 test 用例和 main 块（含三态标记输出）。

### 输入 / 输出契约

| 类型 | 内容 | 需要读取的信息 |
|------|------|---------------|
| 必需输入 | `examples/{op}/DESIGN.md` | 编程模式、API 选型、内存层级、tiling 策略、loop 结构、同步策略、验证方案（含 golden 草案、典型 shape） |
| 输出文件 | `examples/{op}/example_{op}.py` | 单一文件，含：`@tilelang.jit` kernel + 内嵌 golden 函数 + 几个常用 shape 的 test 用例 + main 块（含三态标记输出） |
| 输出文件 | `examples/{op}/README.md`（可选） | 实现说明 |
| 使用 Skill | `tilelang-op-generate` | — |

### Test 用例约定

`example_{op}.py` 的 main 块**直接内嵌测试用例**，覆盖几个常用 shape（不需要单独的 test 文件）：

- 至少包含 3 类用例：基础 shape（最小可运行）、典型 shape（常用业务场景）、边界 shape（极值或对齐边界）
...
| Test 用例覆盖 | main 块中至少 3 个测试用例 | 返回 fail + `insufficient_test_cases` |
...
| 文件 | 生成阶段 | 说明 |
|------|---------|------|
| `example_{op}.py` | Stage 2 / 3 | 单一交付文件：`@tilelang.jit` kernel + 内嵌 golden + 多 shape test 用例 + main（含三态标记） |
