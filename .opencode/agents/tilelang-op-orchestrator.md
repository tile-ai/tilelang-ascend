...
├── example_{op}.py               # Stage 2 产物（含 @tilelang.jit kernel + 内嵌 golden + 几个常用 shape 的 test 用例 + main 块）
...
| `example_{op}.py` | Stage 2/3/4 | Stage 2/3/4 | 单一交付文件：`@tilelang.jit` kernel 实现 + 内嵌 PyTorch golden 函数 + 几个常用 shape 的 test 用例 + main 入口（含三态标记输出） |
...
| 自动工件 | `example_{op}.py`、`README.md` | 可按阶段结果覆盖；Stage 3 修复前必须备份到 `history_version/{op}_impl_s3_attempt{N}.py` |
...
| 4 | 性能调优（**可选**） | 调度 Subagent | `@tilelang-op-perf-tuner` | 精度通过后**主动询问用户**：用户表示需要调优且提供必要信息 |

> **Stage 4 是用户可选阶段**。精度通过（无论是 Stage 2 直接 PASS 还是 Stage 3 修复后 PASS）后，orchestrator 必须**主动询问用户是否需要性能调优**。用户拒绝则直接置 `SUCCESS`，不进入 Stage 4；用户同意则按下文「Stage 4 进入前的用户确认」收集必要信息后进入。判断结果写入状态字段 `perf_tuning_requested`（`yes` / `no`）。
...
| 检测结果 | 含义 | 下一步 |
|----------|------|--------|
| `[PRECISION_PASS]` | 精度通过 | 依次 `complete_stage(2)` → `complete_stage(3)`（跳过精度修复） → **主动询问用户是否需要性能调优**（见「Stage 4 进入前的用户确认」） |
...
精度通过后（Stage 2 直接 PASS 或 Stage 3 修复后 PASS），orchestrator **必须**先与用户交互，再决定是否进入 Stage 4：
...
| 需要 / 是 / yes 等 | 继续询问性能调优必要信息（见下表），收集完成后写 `perf_tuning_requested="yes"` 并进入 Stage 4 |
...
> 信息收集后，orchestrator 把这些内容**追加**写回 `examples/{op}/DESIGN.md` 的"性能目标"章节（不覆盖既有内容），然后再 `start_stage(4)` 进入性能调优。
...
| 状态 | 含义 |
|------|------|
| `SUCCESS` | Stage 4 按中止条件完成 **或** 精度通过后用户表示不需要性能调优 |
