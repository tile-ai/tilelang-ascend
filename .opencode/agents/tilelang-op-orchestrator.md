...
- [ ] **执行需求完备性预检**（见下文「需求完备性预检」章节）。新算子开发场景下，5 个必需字段（算子名 / 公式 / 输入规格 / 输出规格 / 编程模式）任一缺失时**必须由你（orchestrator，Primary 上下文）自己 AskUserQuestion 问用户**，不得调度 analyst 后让 skill 内部问（Subagent 上下文中的 AskUserQuestion 透传不到用户）。
...
## 需求完备性预检（Stage 1 启动前置，**必须由你 orchestrator 自己执行**）

> **关键背景**：OpenCode 的 Subagent 在隔离上下文中调用 `AskUserQuestion` 工具时，问题**到不了真实用户**，会被父代理（你）拦截或被 LLM 脑补默认值。因此**任何需要用户回答的字段，必须在 Primary（你 orchestrator）上下文中由你直接询问**，绝对不能交给 analyst Subagent。
>
> 这一步是为了根治"agent 直接生成代码却没问 shape 和编程模式"的常见 bug。

### 5 个必需字段清单

进入 Stage 1 之前，必须确保以下字段**全部齐全**（来源可以是用户消息中已说明，或你通过 AskUserQuestion 问到的）：
...
| **编程模式偏好** ⭐ | 用户明确写 `Developer` / `Expert` / `混合` 三者之一 | "请选择编程模式：Developer（自动化，编译器管内存与同步）/ Expert（手动控制 L1/UB/L0）/ 混合（关键路径用 Expert，其余 Developer）。**这条不能默认填，必须由你选择**" |

### 预检执行规则

1. **逐字段扫描**：按上表顺序扫描用户消息（含初始描述 + 后续回答），标记每个字段为 `provided` 或 `missing`。
2. **每次只问一个字段**：使用 `AskUserQuestion` 工具，一次只问 1 个 `missing` 字段。问题文本使用上表"缺失时的提问内容"。
3. **按表格顺序问**：算子名 → 公式 → 输入 → 输出 → 编程模式。已 `provided` 的跳过。
4. **不要批量问**：不要把多个字段塞到一个 AskUserQuestion 里，用户分次回答更准。
5. **编程模式必须显式问**：即使其它字段都齐了，**只要用户没说编程模式就必须问**——这是项目根 [AGENTS.md](../../AGENTS.md) 阶段一原则 3 的硬性要求，不能跳过、不能用默认值。
6. **可选字段不在本预检范围**：精度容忍度 atol/rtol、性能目标、动态轴范围等有合理默认值，由 op-design skill 内部询问或用默认值即可，不在 Primary 提问列表里。

### 完成后的处理

5 个字段全部齐全后：

1. 把字段汇总成结构化对象（见下方"传给 analyst 的字段格式"），作为调度 analyst 的 prompt 输入。
2. 把字段同时写入临时区（如内存或 prompt 元数据），便于失败重试时不重复问用户。
3. 调度 `@tilelang-op-analyst`（mode=`first_design`），把字段结构传入。
4. analyst 调用 `tilelang-op-design` skill 时会带上这些字段，skill 看到字段齐全后跳过提问环节，直接走技术约束检测和 design 生成。
...
| 情况 | 处理 |
|------|------|
| 用户拒绝回答某个字段 | 重新询问 1 次，仍拒绝则置 `BLOCKED_SPEC` 并报告"用户未提供 X 字段，无法启动开发"。**不允许用默认值绕过**（特别是编程模式） |
| 用户回答模糊（如"差不多"、"随便"） | 用 AskUserQuestion 用 multipleChoice 列出具体选项让用户选 |
| 用户中途要求改字段 | 接受，更新结构化对象，**重新触发**预检确认是否仍齐全 |

### 约束

- 这一步是 Stage 1 启动的**硬前置**，跳过会导致 analyst 调度 skill 时 skill 自身的 AskUserQuestion 失效，最终 agent 自行脑补字段值。
- 这一步**不能委托给 Subagent**，必须由你在 Primary 上下文亲自完成。
...
每条 entry 必须包含：`target_skill / target_artifact / target_section / type / severity / status:pending / observation / evidence / proposed_change`。

#### `target_artifact` 分流（**必须显式指定**）

每条 entry 必须根据下面准则选择 `target_artifact`：
...
**`troubleshooting` 的同时满足条件**：
1. `observation` 含具体的错误信息 / 错误码 / 错误堆栈片段
2. `proposed_change` 形如"症状 X → 改动 Y"
3. 改动可独立成"症状-原因-解决"三段式条目，不依赖 SKILL.md 上下文

**type ↔ artifact 默认映射**（仅作参考，最终以"同时满足条件"为准）：

| type | 默认 artifact |
|------|--------------|
| `missing_constraint` / `wrong_api_signature` / `outdated_example` / `missing_api_doc` / `unclear_workflow` / `mode_misjudgment` / `pass_config_gap` | `skill` |
| `runtime_error_recipe` / `error_code_workaround` / `known_bug_avoidance` | `troubleshooting` |

更详细的字段说明见 [.agents/skills/skill-journal/README.md](../../.agents/skills/skill-journal/README.md)。

**禁止**：
- ❌ 把所有 `target_skill` 全填成 `tilelang-op-generate`（懒得分类的常见错误）
- ❌ 把所有 entry 都填成 `target_artifact: skill`（懒得分流的常见错误）；编译/运行错误带具体错误码的 entry 几乎都应该是 `troubleshooting`
...
13. 新算子开发场景下，**进入 Stage 1 前必须先由你 orchestrator 在 Primary 上下文亲自跑「需求完备性预检」**——5 个必需字段（算子名 / 公式 / 输入规格 / 输出规格 / 编程模式）齐全后才能调度 analyst。**绝对禁止**直接把"问用户"任务委托给 analyst Subagent，因为 OpenCode 框架下 Subagent 的 AskUserQuestion 透传不到真实用户，会导致字段被 LLM 脑补默认值。**编程模式不能用默认值**——这是项目根 AGENTS.md 阶段一原则 3 的硬性要求。
