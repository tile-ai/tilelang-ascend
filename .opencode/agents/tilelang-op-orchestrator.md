...
`state_transition(action=complete_stage, stage=N)` 抛异常即视为门禁失败。**该工具不会自动累加 retry_count，也不会改写 stage_status**——重试计数完全依赖 Orchestrator 显式调用 `fail_stage`。Orchestrator 必须按以下固定 3 步处理，**禁止跳过任何一步直接调度 Subagent，禁止改而对下一个 Stage 执行 `complete_stage`**：
...
| action | 说明 |
|--------|------|
| `start_stage` | 将目标 stage 标记为 `in_progress`，用于初始化、失败重试或设计回退。若已有其他 stage 处于 `in_progress` 会抛异常，必须先对其调用 `fail_stage` 或 `complete_stage` |
| `complete_stage` | **预校验门禁**后标记完成并自动推进到 N+1。**门禁失败时抛异常且不写状态文件** —— `retry_count` 不会自动累加，必须按「门禁失败处理流程」显式调用 `fail_stage` 才能进入合法重试 |
| `fail_stage` | 记录失败，`stage_retry_count[stage] += 1`，可通过 `start_stage` 重试。当 `reason=design_error` 时配合设计回退使用 |
...
5. 仅允许通过 `state_transition` 工具修改 `examples/{op}/.orchestrator_state.json`，禁止通过 write/edit/multiedit/bash/shell 直接写该文件。
...
8. **绝对禁止 Orchestrator 自行修复代码或编辑工件**：任何阶段返回失败时，Orchestrator 都不得自行编辑代码、修改实现、调整设计或修复精度问题。唯一允许的操作是重新调度对应 Subagent 处理、走设计回退流程、或在重试次数耗尽后标记为 BLOCKED。**例外**：当失败来自工具层（`complete_stage` 抛出的门禁失败），必须先按「门禁失败处理流程」走完 `fail_stage → start_stage` 再调度 Subagent；该流程中的 `state_transition` 调用不属于"自行修复"。
