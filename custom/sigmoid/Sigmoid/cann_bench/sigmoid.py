...
# cann-bench cases where exp_div is safe (|x| <= 10 for fp16, |x| <= 87 for fp32).
# Uses sigmoid(x)=exp(x)/(1+exp(x)) — 3 V ops instead of 5 (saves Muls+Duplicate).
# Cases with polynomial/linear approximations are excluded (they take priority).
...
_FP32_CLAMP_EXP_DIV_SHAPES = {
    # case 5 (8192,8192) [-100,100]: clamp overhead > exp_div savings, keep tile_sig
}
...
def sigmoid(x):
    ...
    # exp_div optimization: use sigmoid(x)=exp(x)/(1+exp(x)) for safe value
    # ranges (saves 2 V ops: Muls + Duplicate). Polynomial/linear approximations
    # take priority over exp_div in the kernel dispatch.
