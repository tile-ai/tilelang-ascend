...
def _estimate_num_blocks(tl_dtype, M, N):
    """Estimate total block count for (M, N) using the same tiling as _select_tiling.

    Used by the ND smart-flatten path to pick the reshape that minimizes
    num_blocks (= launch / dispatch overhead). Returns (num_blocks, num_iters).
    For high-dim small-N shapes (e.g. case 13/20), the default "merge all but
    last dim into M" yields a huge M and tiny N, blowing up block count; picking
    a split point that enlarges N lets _select_tiling use wider block_N and
    fewer blocks.
    """
    if M <= 0 or N <= 0:
...
def mish(x):
    ...
    # For ND tensors, smart-flatten: pick the split point that minimizes
    # num_blocks (see _estimate_num_blocks), which is critical for high-dim
    # small-N shapes (e.g. case 13/20) where the default "merge all but last
    # dim into M" yields a huge M and tiny N, blowing up block count.
    ...
    if x.ndim <= 1:
        ...
        # ND: smart reshape — choose split point that minimizes num_blocks.
        # mish is element-wise, so any flatten (M, N) with M*N=total is valid
        # as long as the tensor is contiguous (reshape is then a zero-copy view;
        # see cann-bench-elementwise-optimization.md §"零拷贝"). We pick the
        # split point that minimizes total block count (= launch overhead),
        # which is the dominant cost for high-dim small-N shapes (cases 13/20).
        # On num_blocks tie, prefer larger split_idx (closer to the original
        # "merge all but last into M" logic, smaller N) to avoid surprising
        # regressions on shapes that were already well-tiled.
        ...
        for s in dims:
            total *= s
        best = None  # (num_blocks, M, N, split_idx)
        # Try every split point: dims[:split_idx+1] -> M, rest -> N.
        # range(len(dims)-1) excludes the degenerate M=total, N=1 case.
        for split_idx in range(len(dims) - 1):
            M = 1
            for s in dims[: split_idx + 1]:
                M *= s
            N = total // M
            if M <= 0 or N <= 0:
                continue
            nb, _ = _estimate_num_blocks(tl_dtype, M, N)
            if best is None or nb < best[0] or (nb == best[0] and split_idx > best[3]):
                best = (nb, M, N, split_idx)
        M, N = best[1], best[2]
        input_2d = x.reshape(M, N)
