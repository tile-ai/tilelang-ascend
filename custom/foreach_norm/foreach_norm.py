...
def _use_direct_norm(scalar: float, n: int, dt: str) -> bool:
    ...
    Linf/Lneg-inf use CANN native amax/amin which is faster than TileLang kernel
    for small single-tensor cases (case 12: 0.442 vs 0.390 with kernel).
    TileLang launch overhead (~10us) + kernel compute > 3 CANN ops (abs+amax+cast).
    """
...
def _finalize_batched(partial: torch.Tensor, scalar: float, out_dtype: torch.dtype) -> torch.Tensor:
    ...
    if scalar == float("inf"):
        ...
        result = partial.sum(dim=reduce_dims).sqrt().sqrt()
    else:
