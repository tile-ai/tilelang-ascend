...
def check_precision(actual: torch.Tensor, golden: torch.Tensor, dtype: str):
    ...
    if m.sum().item() == 0:
        ...
        masks_match = torch.equal(torch.isnan(a), torch.isnan(g)) and torch.equal(torch.isinf(a), torch.isinf(g))
...
L0_CASES = [
    ("l0_l2_fp16_small", "float16", (8192,), 2.0, 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l2_fp16_mid", "float16", (32768,), 2.0, 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l2_fp16_large", "float16", (131072,), 2.0, 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l1_fp16", "float16", (8192,), 1.0, 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l3_fp16", "float16", (8192,), 3.0, 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_linf_fp16", "float16", (8192,), float("inf"), 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l0_count_fp16", "float16", (8192,), 0.0, 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_lneg1_fp16", "float16", (8192,), -1.0, 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l2_fp32_mid", "float32", (32768,), 2.0, 8192, 1, ["D-DTYPE-fp32", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l1_fp32", "float32", (8192,), 1.0, 8192, 1, ["D-DTYPE-fp32", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l2_bf16_mid", "bfloat16", (32768,), 2.0, 8192, 1, ["D-DTYPE-bf16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l1_bf16", "bfloat16", (8192,), 1.0, 8192, 1, ["D-DTYPE-bf16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l2_2d_fp16", "float16", (1024, 1024), 2.0, 8192, 1, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l2_tl2_fp16", "float16", (8192,), 2.0, 8192, 2, ["D-DTYPE-fp16", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
    ("l0_l1_tl3_fp32", "float32", (8192,), 1.0, 8192, 3, ["D-DTYPE-fp32", "D-SHAPE-ALIGNED", "D-PARAM-scalar"]),
]
    ...
    for name, dt, shape, scalar, _bn, ll, _tags in L0_CASES:
        ...
        try:
            ...
            for i, (y, ref) in enumerate(zip(y_list, ref_list)):
                ...
                print(
                    f"[PRECISION_{tag}] l0 {name} shape={shape} dtype={dt} "
...
L1_CASES = [
    ...
    ("l1_edge_fp16_scalar", "float16", (1,), 2.0, 8192, 1, (-1.0, 1.0), ["D-DTYPE-fp16", "D-SHAPE-EDGE", "D-VALRANGE-S", "D-PARAM-scalar"]),
    (
        "l1_edge_fp32_1row",
        "float32",
        (1, 8192),
...
BOUNDARY_CASES = [
    ("b_inf_fp16_l2", "float16", (8192,), 2.0, 1, "inf", ["D-SPECIAL-INF", "D-DTYPE-fp16"]),
    ("b_nan_fp16_l2", "float16", (8192,), 2.0, 1, "nan", ["D-SPECIAL-NAN", "D-DTYPE-fp16"]),
    ("b_zero_fp16_l2", "float16", (8192,), 2.0, 1, "zero", ["D-SPECIAL-ZERO", "D-DTYPE-fp16"]),
    ("b_dbound_fp16_l2", "float16", (8192,), 2.0, 1, "dbound_fp16", ["D-SPECIAL-DBOUND", "D-DTYPE-fp16"]),
    ("b_dbound_fp32_l2", "float32", (8192,), 2.0, 1, "dbound_fp32", ["D-SPECIAL-DBOUND", "D-DTYPE-fp32"]),
    ("b_inf_bf16_linf", "bfloat16", (1000003,), float("inf"), 1, "inf", ["D-SPECIAL-INF", "D-DTYPE-bf16", "D-SHAPE-PRIME"]),
    ("b_zero_fp32_l0count", "float32", (8192,), 0.0, 1, "zero", ["D-SPECIAL-ZERO", "D-DTYPE-fp32", "D-PARAM-scalar"]),
]
    ...
    for name, dt, shape, scalar, _bn, ll, vrange, _tags in L1_CASES:
        ...
        try:
            ...
            for i, (y, ref) in enumerate(zip(y_list, ref_list)):
                ...
                tag = "PASS" if passed else "FAIL"
                print(
                    f"[PRECISION_{tag}] l1 {name} shape={shape} dtype={dt} "
                    f"scalar={scalar} tl_idx={i}/{ll} "
                    f"matched_ratio={ratio:.4f} max_abs={max_abs:.3e}"
                )
                if not passed:
    ...
    for name, kind, _tags in L2_CASES:
        try:
            if kind == "unsupported_dtype_float64":
                ...
                x = [torch.randn((8192,), dtype=torch.float16, device="npu"), torch.randn((8192,), dtype=torch.float32, device="npu")]
                foreach_norm(x, 2.0)
...
def test_foreach_norm_boundary():
    ...
    for name, dt, shape, scalar, ll, vkind, _tags in BOUNDARY_CASES:
        ...
        try:
            if vkind == "zero":
                x_list = [torch.zeros(shape, dtype=torch_dt, device="npu") for _ in range(ll)]
            else:
                x_list = [torch.zeros(shape, dtype=torch_dt, device="npu") for _ in range(ll)]
                for t in x_list:
                    n = min(10, t.numel())
            ...
            for i, (y, ref) in enumerate(zip(y_list, ref_list)):
                ...
                print(
                    f"[BOUNDARY_{tag}] boundary {name} shape={shape} dtype={dt} "
                    f"scalar={scalar} tl_idx={i}/{ll} "
                    f"matched_ratio={ratio:.4f} max_abs={max_abs:.3e}"
                )
                if not passed:
