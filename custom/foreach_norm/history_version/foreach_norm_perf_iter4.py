...
def foreach_norm(x_list: List[torch.Tensor], scalar: float) -> List[torch.Tensor]:
    ...
    for n, indices in groups.items():
        ...
        if batch == 1:
            # Single tensor: direct call (no stack overhead)
            x_flat = x_list[indices[0]].view(-1)
            kernel = _get_kernel(scalar, 1, n, block_n, launch_cores, first_dt)
            partial = kernel(x_flat.unsqueeze(0))  # (1, launch_cores)
            result = _finalize_batched(partial, scalar, torch_dt)  # (1,)
            results[indices[0]] = result[0].view(())
        else:
