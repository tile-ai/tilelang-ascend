...
def _smart_flatten(shape, dtype_str):
    ...
    Evaluation uses fixed 128×128 tiling (Vector sweet spot) to compute
    ...
    This avoids regression on cases like [3,7,13,4001] where split_idx=1
    (M=21 < 128) has fewer actual num_blocks (77) but lower Vector utilization
    than split_idx=2 (M=273 >= 128, num_blocks=96). Using 128×128 evaluation,
    split_idx=2 (96) is preferred over split_idx=1 (407), avoiding the 12%
    regression from small block_M.
    ...
    for split_idx in range(len(shape) - 1):
        ...
        M = M_acc
        N = total // M
        # Evaluate with fixed 128×128 (Vector sweet spot) — prefers M >= 128 splits.
        # Actual execution may use different tiling (large block_N for small M).
