...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "validate":
        ...
        if unregistered:
            ...
            print(
                "every operator is reserved in ci/operator_test_manifest.yaml, so a "
                "test that matches nothing is usually a misspelt file name; its "
                "reservation would stay pending and the operator would keep running "
                "in the legacy runner",
                file=sys.stderr,
            )
            return 1
