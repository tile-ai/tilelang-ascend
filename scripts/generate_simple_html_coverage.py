...
def generate_simple_html(coverage_json: Path, file_path: str, output_dir: Path):
    ...
    html_lines = [
        ...
        '<head>',
        '<meta charset="utf-8">',
        '<title>Coverage: {}</title>'.format(file_path),
        '<style>',
        ...
        # Title
        '<h1>Coverage Report: {}</h1>'.format(file_path),
        
        # Summary
        '<div class="summary">',
        '<p><b>Coverage: {:.2f}%</b></p>'.format(file_data['summary']['percent_covered']),
        '<p>Executed: {} / {} lines</p>'.format(
    ...
    for i, line in enumerate(source_lines, 1):
        ...
        html_lines.append(
            '<span class="line {}"><span class="lineno">{}</span>{}</span>'.format(
                css_class, i, escaped_line
            )
        )
    ...
    output_file = output_dir / '{}.html'.format(safe_name)
