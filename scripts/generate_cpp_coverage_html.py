...
def generate_cpp_html_report(source_file: str, line_exec: dict, output_file: Path):
    ...
    html = [
        ...
        '<head>',
        '<meta charset="utf-8">',
        '<title>C++ Coverage: {}</title>'.format(Path(source_file).name),
        '<style>',
        ...
        '<h2>{}</h2>'.format(source_file),
        '<div class="summary">',
        '<b style="font-size:18px">Coverage: {:.2f}%</b><br>'.format(percent),
        'Executed: {} / {} lines<br>'.format(covered, total),
        'Missing: {} lines<br><br>'.format(total - covered),
    ...
    for i, line in enumerate(source_lines, 1):
        ...
        html.append('<span class="line {}"><span class="lineno">{}</span><span class="count">{:>8}</span>{}</span>'.format(css, i, count, escaped))
