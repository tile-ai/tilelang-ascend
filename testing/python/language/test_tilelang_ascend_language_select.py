...
def select_kernel(M, N, block_M, block_N, dtype="float16"):
    ...
    def main(
        ...
        with T.Kernel(m_num * n_num, is_npu=True) as (cid, vid):
            ...
            with T.Scope("V"):
                ...
                T.copy(Mask[offset_m : offset_m + sub_block_m, 
                            offset_mask_n : offset_mask_n + block_mask_width], mask_ub)
...
def run_test(M, N, block_M, block_N, target):
    device = "npu"
    torch.manual_seed(0)
    tilelang.cache.clear_cache()

    # 1. Compile the operator
    func_def = select_kernel(M, N, block_M, block_N, dtype="float16")
...
def test_select_op(target, shape):
    ...
    run_test(M, N, 128, 256, target=target)
...
if __name__ == "__main__":
    ...
    run_test(args.m, final_n, 128, 256, target=args.target)
