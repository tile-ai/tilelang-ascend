...
def test_select_tensor_op(dtype, target, shape):
    if len(shape) == 1:
        ...
        if N % 8 != 0:
            pytest.skip("N must be multiple of 8")
        run_test_mod2(M, N, 128, 256, dtype, target)
...
def test_select_scalar_op(dtype, target, shape):
    if len(shape) == 1:
        ...
        if N % 8 != 0:
            pytest.skip("N must be multiple of 8")
        run_test_mod1(M, N, 128, 256, dtype, target)


@pytest.mark.parametrize("dtype", ["float16", "float32"])
def test_select_cmpmask_op(dtype):
    run_test_mod3(dtype)
...
if __name__ == "__main__":
    ...
    run_test_mod1(args.m, final_n, 128, 256, dtype=args.dtype, target=args.target)
    run_test_mod2(args.m, final_n, 128, 256, dtype=args.dtype, target=args.target)
    run_test_mod3(dtype=args.dtype)
