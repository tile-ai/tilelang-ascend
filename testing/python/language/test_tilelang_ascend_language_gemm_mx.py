            ...
            with T.Kernel(m_num * n_num, is_npu=True) as (cid, _):
                ...
                with T.Scope("C"):
                    ...
                    for k in T.serial(loop_k):
                        ...
                        T.copy(
                            A_scale[bx * block_M : (bx + 1) * block_M, k * kScale_per_iter : (k + 1) * kScale_per_iter, :],
                            SA_L1,
                            src_layout="MX_A_ND",
                        )
                        T.copy(
                            B_scale[k * kScale_per_iter : (k + 1) * kScale_per_iter, by * block_N : (by + 1) * block_N, :],
                            SB_L1,
                            src_layout="MX_B_ND",
                        )
...
@pytest.mark.parametrize(
    "M,N,K,block_M,block_N,K_L1,scale_mode,format",
    [
        # Basic tests (E5M2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="random",
        choices=["uniform_identity", "uniform_2x", "random_narrow", "random_wide", "random"],
        help="Scale generation mode",
    )
    parser.add_argument("--format", type=str, default="e5m2", choices=["e5m2", "e4m3"], help="FP8 data format")
    ...
    run_test(args.m, args.n, args.k, args.block_m, args.block_n, args.k_l1, args.scale_mode, format=args.format)
