...
def _run_ring_slot_clear(target, M=64, N=64, K=200, CHUNK=80):
                ...
                with T.Scope("C"):
                    ...
                    for i in T.serial((m_num * n_num + CORE_NUM - 1) // CORE_NUM):
                        ...
                        if task < m_num * n_num:
                            ...
                            T.wait_flag("mte1", "mte2", 0)
                            T.copy(
                                A[bx * BLOCK_M : (bx + 1) * BLOCK_M, 0:CHUNK],
                                A_L1[0, :, :],
                            )
                            T.copy(
                                B[0:CHUNK, by * BLOCK_N : (by + 1) * BLOCK_N],
                                B_L1[0, :, :],
                            )
    ...
    # The second ring slot is selected at runtime.  Its primary-copy decision
    # must therefore remain a generated runtime tile-alignment predicate.
    assert "% 5120 == 0" in compiled.get_kernel_source()
