...
# ==========================================
# Cube sliced copy test:
# GM -> L1 via npuir_load_nd2nz, compute on Cube to L0C,
# then store back to GM via npuir_store_fixpipe (sliced on M dimension)
# ==========================================
@tilelang.jit(out_idx=[-1], target="npuir")
def test_cube_sliced_copy_2d(block_M, block_N, slice_M, dtype="float16"):
    @T.prim_func
    def main(
        In: T.Tensor((block_M, block_N), dtype),
        K: T.Tensor((block_N, block_N), dtype),
        Out: T.Tensor((block_M, block_N), dtype),
    ):
        with T.Kernel(1, is_npu=True) as (idx_, _):
            # L1/L0C buffer allocation follows flash_attn expert style
            l1_a = T.alloc_L1((slice_M, block_N), dtype)
            l1_b = T.alloc_L1((block_N, block_N), dtype)
            l0_c = T.alloc_L0C((slice_M, block_N), "float32")

            with T.Scope("Cube"):
                # 1) GM -> L1 using npuir_load_nd2nz (sliced on M dimension)
                T.npuir_load_nd2nz(In[0, 0], l1_a, [slice_M, block_N])
                T.npuir_load_nd2nz(K[0, 0], l1_b, [block_N, block_N])

                # 2) Simple Cube matmul on L0C: Out_slice = In_slice @ I = In_slice
                T.npuir_dot(
                    l1_a,
                    l1_b,
                    l0_c,
                    initC=True,
                    size=[slice_M, block_N, block_N],
                )

                # 3) L0C -> GM using npuir_store_fixpipe, only update sliced rows
                with T.rs("PIPE_FIX"):
                    T.npuir_store_fixpipe(
                        l0_c,
                        Out[0, 0],
                        size=[slice_M, block_N],
                        enable_nz2nd=True,
                    )
    return main
...
def test_cube_2d():
    print("\n" + "=" * 30 + " Running Cube Sliced Copy 2D Test " + "=" * 30)
    block_M, block_N, slice_M = 32, 64, 8

    kernel = test_cube_sliced_copy_2d(block_M, block_N, slice_M)

    In = torch.ones(block_M, block_N).npu().half()
    K = torch.eye(block_N).npu().half()
    Out = torch.zeros(block_M, block_N).npu().half()

    kernel(In, K, Out)

    expected_top = torch.ones(slice_M, block_N).npu().half()
    expected_bottom = torch.zeros(block_M - slice_M, block_N).npu().half()

    # 仅前 slice_M 行被更新为 1，其余行为 0，验证 sliced copy 流程
    torch.testing.assert_close(Out[:slice_M], expected_top, rtol=1e-5, atol=1e-5)
    torch.testing.assert_close(Out[slice_M:], expected_bottom, rtol=1e-5, atol=1e-5)

    print("Cube Sliced Copy 2D Test Passed!")


def main():
    # 专注测试 Cube 核上 GM->L1(npuir_load_nd2nz)、Cube 计算到 L0C、
    # 再通过 npuir_store_fixpipe sliced 写回 GM 的流程
    test_cube_2d()
