        ...
        with T.Kernel(b * n, is_npu=True) as (cid, _):
            with T.Scope("Cube"):
                ...
                for i in T.serial(T.ceildiv(d, block_d)):
                    ...
                    T.npuir_load_nd2nz(Q[b_id, n_id, 0, i * block_d], Q_BUF, [s, real_block_d])
                    T.npuir_load_nd2nz(K[b_id, n_id, 0, i * block_d], K_BUF, [s, real_block_d])

                    T.npuir_dot(Q_BUF, K_BUF, A_BUF, initC=(i == 0), b_transpose=True, size=[s, real_block_d, s])
