...
def simple_copy_2d(M, N, block_M, block_N, dtype="float16", accum_dtype="float32"):
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(N, block_N) * T.ceildiv(M, block_M), is_npu=True) as (cid, _):
            ...
            # B1_frag = T.alloc_fragment((block_M, block_N), dtype)
            # B2_frag = T.alloc_fragment((block_M, block_N), dtype)
            C_frag = T.alloc_fragment((block_M, block_N), accum_dtype)
            tmp = T.alloc_fragment((3, block_N), dtype)

            T.copy(In[by * block_M, bx * block_N], A_frag)
            T.copy(A_frag, B_frag)
            T.copy(A_frag[1:3,:], tmp[0:2, :])
            T.npuir_add(A_frag[1:3,:], A_frag[1:3,:], B_frag[1:3,:])
            T.copy(B_frag, C_frag)  # ub to ub with cast
...
def test_2d():
    ...
    kernel = simple_copy_2d(64, 64, 64, 64)

    input = torch.randn(64, 64).npu().half()
    a = torch.zeros(64, 64).npu().half()
    b = torch.zeros(64, 64).npu().half()
    c = torch.zeros(64, 64).npu()
