...
def nz2nd(M, N, K, block_M, block_N, dtype="float16", inner_dtype="float32"):
    m_num = M // block_M
    n_num = N // block_N
        ...
        with T.Kernel(m_num * n_num, is_npu=True) as (cid, subid):
            with T.Scope("Cube"):
                ...
                with T.rs("PIPE_FIX"):
                    ...
                    T.npuir_store_nz2nd(C_BUF, C[bx, by], [tile_size_M, tile_size_N])
                    T.sync_block_set(0)
                    T.block_barrier(0)

                with T.rs("PIPE_FIX"):
                    for _i in range(0, FFTS_FLAG_THRESHOLD):
                        T.sync_block_wait(1)
