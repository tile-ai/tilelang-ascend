        ...
        with redirect_stdout(f), redirect_stderr(f):
            ...
            try:
                ...
                def supply_prog(params):
                    ...
                    State = torch.randn(num_tokens, num_heads, 32, dtype=torch.float32).npu()
                ...
                def rotary_embedding(num_tokens, num_heads, head_dim, BLOCK_N, BLOCK_H):
                    ...
                    def rotary_embedding_kernel(
                        ...
                        with T.Kernel(T.ceildiv(num_tokens, BLOCK_N) * T.ceildiv(num_heads, BLOCK_H), is_npu=True) as (cid, _):
                            by = cid // T.ceildiv(num_tokens, BLOCK_N)
                            bx = cid % T.ceildiv(num_tokens, BLOCK_N)
                            half = head_dim // 2

                            X_shared = T.alloc_shared((BLOCK_N, BLOCK_H, head_dim // 2), dtype)
                            Y_shared = T.alloc_shared((BLOCK_N, BLOCK_H, head_dim // 2), dtype)
                            ...
                            for i in T.serial(BLOCK_N):
                                ...
                                if token_idx < num_tokens:
                                    T.copy(Cos[token_idx, 0, 0], Cos_shared[i, 0], size=[1, half])
                                    T.copy(Sin[token_idx, 0, 0], Sin_shared[i, 0], size=[1, half])

                            for i, h in T.Parallel(BLOCK_N, BLOCK_H):
                                token_idx = bx * BLOCK_N + i
                                head_idx = by * BLOCK_H + h
                                if token_idx < num_tokens and head_idx < num_heads:
                                    T.copy(State[token_idx, head_idx, 0], X_shared[i, h, 0], size=[1, 1, half])
                                    T.copy(State[token_idx, head_idx, half], Y_shared[i, h, 0], size=[1, 1, half])
                            ...
                            for i, h in T.Parallel(BLOCK_N, BLOCK_H):
                                ...
                                head_idx = by * BLOCK_H + h
                                if token_idx < num_tokens and head_idx < num_heads:
                                    T.copy(X_shared[i, h, 0], Out[token_idx, head_idx, 0], size=[1, 1, half])
                                    T.copy(Y_shared[i, h, 0], Out[token_idx, head_idx, half], size=[1, 1, half])
                ...
                # trigger autotune
                # State = torch.randn(num_tokens, num_heads, dtype=torch.float32).npu()
                # Cos = torch.randn(num_tokens, dtype=torch.float32).npu()
                # Sin = torch.randn(num_tokens, dtype=torch.float32).npu()

                # out = func(State, Cos, Sin)
