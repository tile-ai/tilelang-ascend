...
def test_vec_tanh(dtype):
    ...
    A = generate_tensor_new(
        shape = (M, N),
        dtype = tc.resolve_dtype(dtype),
        data_range = (-1.0, 1.0),
    ).npu()
    B = torch.zeros((M, N), dtype = tc.resolve_dtype(dtype)).npu()
    ...
    tc.assert_close(B.cpu(), ref_cpu, rtol=1e-2, atol=1e-2)
